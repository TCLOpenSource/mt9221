...
void __init setup_log_buf(int early)
{
	...
	if(log_buf)pa=virt_to_phys(log_buf)-(phys_addr_t)0X20000000;
	pr_err("DEBUG log_buf addr-0x20000000 :%pad,len:0x%x.\n",(&pa),log_buf_len);
	...
	if(log_buf)pa=virt_to_phys(log_buf);
	pr_err("DEBUG new_log_buf addr:%pad,len:0x%x.\n",(&pa),log_buf_len);
...
#if defined(CONFIG_MP_DEBUG_TOOL_LOG_PREFIX)
	/*printk prefix {*/
	/* this_cpu is unsafe here, but it's ok for log print */
	int this_cpu = raw_smp_processor_id();
	if (irqs_disabled())

...
//#define R842_Delay_MS(x) msleep(x)
...
UINT8   R842_XTAL_CAP_VALUE = 13;    //13pF
#else
UINT8   R842_XTAL_CAP_VALUE = 38;    //range 0~41
#endif
UINT8   R842_XTAL_CAP = 0;
...
UINT8 R842_iniArray[3][R842_REG_NUM] = {
    {     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  //24M
      //   0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07
           0x80, 0xFE, 0x1C, 0x90, 0x60, 0x26, 0x00, 0xAC, 
...
UINT8 R842_IMR_CAL_Array[3][R842_REG_NUM] = {   //24M/16M/27M
    { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  //read only
       ...
       0x63, 0xF8, 0xC2, 0x45, 0x55, 0xFB, 0x6C, 0x01 },  //R[41]=0xF6->0xF8; mixer_gain=8
...
static UINT8 R842_Atsc_Mode = 0;
static UINT8 R842_Atsc_Mode_Pre = 0;
static UINT8 R842_Atsc_Mode1_Count = 0;
static UINT8 R842_Atsc_Mode2_Count = 0;
static UINT8 R842_Atsc_LPF = 0;
static UINT8 R842_Atsc_HPF = 0;
//0(8M), 2(7M), 3(6M)
...
//                   Internal Functions                     //
//----------------------------------------------------------//
void R842_Delay_MS(UINT16 DelayTimeInMs) ;
...
// I2C functions
//============================================================================//
R842_BOOL I2C_Write_Len(I2C_LEN_TYPE *I2C_Info);
R842_BOOL I2C_Read_Len(I2C_LEN_TYPE *I2C_Info);
R842_BOOL I2C_Write(I2C_TYPE *I2C_Info);
...
R842_Sys_Info_Type R842_Sys_Sel(R842_Standard_Type R842_Standard)
{
...
#if(R842_ALL_IMR_NOR==TRUE)
        R842_Sys_Info.FILT_CAL_IF=7650;      //CAL IF
        R842_Sys_Info.HPF_COR=5;             //R24[3:0]=6
#else
        R842_Sys_Info.FILT_CAL_IF=7850;      //CAL IF
    ...
    case R842_DTMB_8M_IF_5M:
        ...
        R842_Sys_Info.HPF_COR=14;            //R24[3:0]=14
#else
        R842_Sys_Info.FILT_CAL_IF=8800;      //CAL IF
        R842_Sys_Info.HPF_COR=10;            //R24[3:0]=10
#endif
    ...
    case R842_DVB_T2_1_7M_IF_5M:
        ...
        if(RF_freq<R842_LNA_MID_LOW[R842_SetTfType])  //low band (270n)
        {
            ...
            switch(R842_Standard) //DVBT2_7M
            {
                ...
                //case R842_DVB_T2_8M_IF_5M:
                //case R842_DVB_T2_1_7M_IF_5M:
                if((RF_freq>=194500) && (RF_freq<202500)) // 198.5
                {
                    R842_SysFreq_Info.MIXER_AMP_LPF=3;        // 3
                    R842_SysFreq_Info.HPF_COMP=0;             // normal
                }
                else
                {
...
//                R842_SysFreq_Info.NRB_BW_LPF=0x03;        // Nrb LPF BW=lowest        (R42[1:0]=2'b11)
        ...
        //overwrite for High band BW 6MHz
                switch(R842_Standard)
                {
                    case R842_DVB_T_6M:
                    case R842_DVB_T_6M_IF_5M:
                    ...
                    R842_SysFreq_Info.LNA_VTL_H=0x7C;         //LNA VTL/H=1.04/1.54       (R38=0x7C)
                    R842_SysFreq_Info.NRB_BW_LPF=0x03;        // Nrb LPF BW=lowest           (R42[1:0]=2'b11)
                    R842_SysFreq_Info.MIXER_GAIN_LIMIT=0x40;  // Mixer Gain Limit=10
                    R842_SysFreq_Info.IMG_NRB_ADDER=0x0C;     // top+11dB
                    break;
...
//        R842_SysFreq_Info.LNA_VTL_H=0x5A;         // LNA VTL/H=0.84/1.34    (R38=0x5A)
    ...
    case R842_ISDB_T_IF_5M:
        ...
        else  //high band
        {
            if(RF_freq<620000)
            {
                ...
                R842_SysFreq_Info.Q_CTRL=0;                //1.5k off
                R842_SysFreq_Info.ENB_ATT=3;               //normal
                R842_SysFreq_Info.MIXER_TOP=15;             //MIXER TOP=9
                R842_SysFreq_Info.HYS_MIXTOP=3;            //top<not change>
            }
            else // >620M
            {
	...
	case R842_ATSC_IF_5M:
		...
		else  //high band
		{
				...
				R842_SysFreq_Info.Q_CTRL=0;               //1.5k off  
				R842_SysFreq_Info.ENB_ATT=3;              //normal
				R842_SysFreq_Info.LNA_TOP=5;              //LNA TOP=5
				R842_SysFreq_Info.RF_TOP=6;               //RF TOP=6   
				R842_SysFreq_Info.NRB_TOP=6;                 //Nrb TOP=6		
				//R842_SysFreq_Info.MIXER_TOP=9;            //MIXER TOP=9
				R842_SysFreq_Info.MIXER_TOP=15;            //MIXER TOP=15
				R842_SysFreq_Info.HYS_MIXTOP=3;           //top<not change> 
				R842_SysFreq_Info.IMG_NRB_ADDER=0x08;     //top+9dB
				R842_SysFreq_Info.NRB_BW_HPF=0x08;       // Nrb HPF BW=high    (R42[3:2]=2'b00)
				//R842_SysFreq_Info.MIXER_GAIN_LIMIT=0x00;  //Mixer Gain Limit=6   (R20[6:5]=2'b00)
				R842_SysFreq_Info.MIXER_GAIN_LIMIT=0x20;  //Mixer Gain Limit=8
		}
...
//overwrite
	...
	{		
		...
		switch(R842_Standard)
		{
			case R842_ATSC:
			case R842_ATSC_IF_5M:
				R842_SysFreq_Info.LNA_DIS_SLOW_FAST=0x03;//0.6u/0.9u 
				break;
			default:
				break;
		}
...
R842_ErrCode R842_Init(void)
{
	...
	{
		R842_XTAL_CAP = R842_XTAL_CAP_VALUE;
	}
    ...
    if(R842_Initial_done_flag==FALSE)
    {
		  ...
		  else if(R842_SHARE_XTAL==R842_MASTER_TO_SLAVE_XTAL_OUT)
          {
              R842_Xtal_Pwr = R842_XTAL_HIGHEST;
          }
          else
...
R842_ErrCode R842_InitReg(void)
{
    ...
    else if(R842_SHARE_XTAL==R842_MASTER_TO_SLAVE_XTAL_OUT) //gm=2.0; xtal power=highest
    {
        R842_iniArray[0][32]=(R842_iniArray[0][32] & 0xE1) | 0x00;
        R842_iniArray[1][32]=(R842_iniArray[1][32] & 0xE1) | 0x00;
        R842_iniArray[2][32]=(R842_iniArray[2][32] & 0xE1) | 0x00;
    }
    ...
    Cap_xx = CapTot & 0x01;
     ...
     switch(u1CalFlag)
     {
        case R842_IMR_CAL:
                ...
                {
                    R842_IMR_CAL_Array[0][32]=(R842_IMR_CAL_Array[0][32] & 0xE1) | 0x00;
                    R842_IMR_CAL_Array[1][32]=(R842_IMR_CAL_Array[1][32] & 0xE1) | 0x00;
                    R842_IMR_CAL_Array[2][32]=(R842_IMR_CAL_Array[2][32] & 0xE1) | 0x00;
                }
        ...
        case R842_TF_MID_LNA_CAL:
                ...
                {
                    R842_TF_MID_CAL_Array[0][32]=(R842_TF_MID_CAL_Array[0][32] & 0xE1) | 0x00;
                    R842_TF_MID_CAL_Array[1][32]=(R842_TF_MID_CAL_Array[1][32] & 0xE1) | 0x00;
                    R842_TF_MID_CAL_Array[2][32]=(R842_TF_MID_CAL_Array[2][32] & 0xE1) | 0x00;
                }
        ...
        case R842_TF_LNA_CAL:
                ...
                {
                    R842_TF_CAL_Array[0][32]=(R842_TF_CAL_Array[0][32] & 0xE1) | 0x00;
                    R842_TF_CAL_Array[1][32]=(R842_TF_CAL_Array[1][32] & 0xE1) | 0x00;
                    R842_TF_CAL_Array[2][32]=(R842_TF_CAL_Array[2][32] & 0xE1) | 0x00;
                }
        ...
        case R842_LPF_CAL:
                ...
                {
                    R842_LPF_CAL_Array[0][32]=(R842_LPF_CAL_Array[0][32] & 0xE1) | 0x00;
                    R842_LPF_CAL_Array[1][32]=(R842_LPF_CAL_Array[1][32] & 0xE1) | 0x00;
                    R842_LPF_CAL_Array[2][32]=(R842_LPF_CAL_Array[2][32] & 0xE1) | 0x00;
                }
...
//read VCO and check again when next channel is gm=1.5, low
    ...
    if((XtalPwrTemp<=2) && (R842_Standard!=R842_STD_SIZE)) //gm=1.5, low
    {
        while((vco_bank_lock==0) && (XtalPwrTemp<7))
        {
            ...
            //set manual VCO bank;
            R842_I2C.RegAddr = 30;
            save_bnak_oth_set = R842_Array[R842_I2C.RegAddr];
            if(vco_bank_read<32)
            ...
            R842_I2C.RegAddr = 30;
            R842_Array[R842_I2C.RegAddr] = save_bnak_oth_set;
            R842_I2C.Data = R842_Array[R842_I2C.RegAddr];
            if(I2C_Write(&R842_I2C) != R842_Success)
                return R842_Fail;
 ...
 //move to end of R842_SetFreq( )??
    ...
    if(R842_SHARE_XTAL!=R842_SLAVE_XTAL_OUT)
    {
        ...
        if( (R842_I2C_Len.Data[2] & 0x40) == 0x00 )
        {
            ...
            {
                R842_I2C.RegAddr = 32;
                R842_Array[32] = (R842_Array[32] & 0xF9) | 0x00;  //pw highest
                R842_I2C.Data = R842_Array[32];
                if(I2C_Write(&R842_I2C) != R842_Success)
                    return R842_Fail;
            }
...
R842_ErrCode R842_SetTF(UINT32 u4FreqKHz, UINT8 u1TfType)
{
    ...
    {
        if(u4FreqKHz < R842_TF_Freq_Low[u1TfType][R842_TF_LOW_NUM-1])
        {
            u1FreqCount = R842_TF_LOW_NUM;
        }
        else
        {
    ...
    {
        if(u4FreqKHz < R842_TF_Freq_Mid[u1TfType][R842_TF_MID_NUM-1])
        {
            u1FreqCount = R842_TF_MID_NUM;
        }
        else
        {
    ...
    {
        if(u4FreqKHz < R842_TF_Freq_High[u1TfType][R842_TF_HIGH_NUM-1])
        {
            u1FreqCount = R842_TF_HIGH_NUM;
        }
        else
        {
...
R842_ErrCode R842_SetStandard(R842_Standard_Type RT_Standard)
{
    ...
    if(RT_Standard != R842_pre_standard)
    {
        ...
        if(I2C_Write(&R842_I2C) != R842_Success)
            return R842_Fail;
        R842_Atsc_LPF = (R842_Fil_Cal_code[RT_Standard]<<1) | (R842_Fil_Cal_LpfLsb[RT_Standard]);
        ...
        if(I2C_Write(&R842_I2C) != R842_Success)
            return R842_Fail;
        R842_Atsc_HPF = R842_Sys_Info1.HPF_COR;
...
R842_ErrCode R842_SetFrequency(R842_Set_Info R842_INFO)
{
	 ...
	 UINT8   RF_gain1_b, RF_gain2_b, RF_gain3_b, RF_gain4_b;
	 UINT8   Agc_clk, Mixandfilt_disch,Gain_same_flag=0;
	 UINT16  Gain_delay=0,Gain_delay_total=0;
...
*/
        ...
        if(R842_INFO.RF_KHz>R842_LNA_HIGH_MID[R842_SetTfType]) //high band
        {
             //RF_buf_cur=high; MixerAmp_cur=low
            R842_I2C.RegAddr = 9;
            R842_Array[9] = (R842_Array[9] & 0xD7) | 0x00;
            R842_Array[9] = (R842_Array[9] & 0xD7) | 0x20;
            R842_I2C.Data = R842_Array[9];
            if(I2C_Write(&R842_I2C) != R842_Success)
                return R842_Fail;
            ...
            //LO_buffer_dc=high
            R842_I2C.RegAddr = 17;
            R842_Array[17] = (R842_Array[17] & 0xFB) | 0x00;
            R842_I2C.Data = R842_Array[17];
            if(I2C_Write(&R842_I2C) != R842_Success)
                return R842_Fail;
            }
            ...
            {
            //RF_buf_cur=low; Mixer_cur=low
                R842_I2C.RegAddr = 9;
                R842_Array[9] = (R842_Array[9] & 0xD7) | 0x28;
                R842_I2C.Data = R842_Array[9];
                if(I2C_Write(&R842_I2C) != R842_Success)
                    return R842_Fail;
            ...
            //LO_buffer_dc=low
                R842_I2C.RegAddr = 17;
                R842_Array[17] = (R842_Array[17] & 0xFB) | 0x04;
                R842_I2C.Data = R842_Array[17];
                if(I2C_Write(&R842_I2C) != R842_Success)
                    return R842_Fail;
            }
    ...
    if(R842_INFO.R842_Standard==R842_DTMB_8M_4570 || R842_INFO.R842_Standard==R842_DTMB_8M_IF_5M)
    {
        if((R842_INFO.RF_KHz==482000) || (R842_INFO.RF_KHz==658000) || (R842_INFO.RF_KHz==786000))
            R842_Array[15] = (R842_Array[15] & 0xF7) | 0x00;
        else
            R842_Array[15] = (R842_Array[15] & 0xF7) | 0x08;
    }
    else
    {
        R842_Array[15] = (R842_Array[15] & 0xF7) | 0x08;
	...
	switch(R842_INFO.R842_Standard)
	{
	  case R842_ATSC:
	  case R842_ATSC_IF_5M:
		if(R842_INFO.RF_KHz>=500000&&R842_INFO.RF_KHz<=600000)
			R842_Array[9] = (R842_Array[9] & 0xFC);
		 break;
	 default:
			 break;
	}
...
//    R842_Delay_MS(delay_tmp);  //for LGIT test, 10 is ok
    ...
    if(I2C_Write(&R842_I2C) != R842_Success)
	 ...
	 if(R842_INFO.R842_Standard<R842_ATV_SIZE)  //ATV
		 R842_Array[46] = (R842_Array[46] | 0x40);
	 else if((R842_INFO.R842_Standard==R842_ATSC||R842_INFO.R842_Standard==R842_ATSC_IF_5M))
		 R842_Array[46] = (R842_Array[46] & 0x40);
	 else
	 ...
	 R842_Atsc_Mode = 0;
	 R842_Atsc_Mode_Pre = 0;
	 R842_I2C_Len.RegAddr = 0;
	 R842_I2C_Len.Len = 49;
	 I2C_Read_Len(&R842_I2C_Len);
	 RF_gain1_b = (R842_I2C_Len.Data[3] & 0x1F);          //lna
	 RF_gain2_b = (R842_I2C_Len.Data[4] & 0x0F);          //rf
	 RF_gain3_b = (R842_I2C_Len.Data[4] & 0xF0)>>4;       //mixer
	 RF_gain4_b = (R842_I2C_Len.Data[5] & 0x0F);          //filter
	 Agc_clk =  (R842_I2C_Len.Data[47] & 0x0C)>>2;          
	 Mixandfilt_disch =  (R842_I2C_Len.Data[30] & 0x03);   
	 if(Mixandfilt_disch<2)
	 {
		if(Agc_clk==0)
			Gain_delay = 4;
		else if (Agc_clk==1)	
	...
	while(Gain_same_flag==0)
	{
		R842_Delay_MS(Gain_delay); //
		R842_I2C_Len.RegAddr = 0;
		R842_I2C_Len.Len = 6;
		I2C_Read_Len(&R842_I2C_Len);
		...
		if((RF_gain1==RF_gain1_b)&&(RF_gain2==RF_gain2_b)&&(RF_gain3==RF_gain3_b)&&(RF_gain4==RF_gain4_b))
		{
			Gain_same_flag = 1;
			Gain_delay_total = Gain_delay_total+Gain_delay;
		}
		else
		{
			RF_gain1_b = RF_gain1;
...
R842_ErrCode R842_Dump_Data(void)
{
    ...
    printk("Start Tuner Log...\n");
    printk("LNA=%d, RF=%d, Mixer=%d, Filter=%d\n", RF_gain1, RF_gain2, RF_gain3, RF_gain4);
    printk("RF_flag=%d, Pulse_flag=%d\n", R842_RfFlag, R842_PulseFlag);
...
R842_ErrCode R842_AtscMonitor(UINT32 Freq_KHz)
{
    UINT8 filter_gain = 0;
    UINT8 mixer_gain = 0;

    if(R842_Atsc_Mode==0) //fast
    {
        R842_Atsc_Mode = 1; //slow_narrow
    }
    ...
    if((Freq_KHz>500000) && (Freq_KHz<600000))
    {
        ...
        filter_gain = (R842_I2C_Len.Data[5] & 0x0F);          //filter
        mixer_gain = (R842_I2C_Len.Data[4] & 0xF0)>>4;        //mixer

        //monitor in which mode
        if(filter_gain<=4)  //mode2 (slow_wide)
        {
            if(R842_Atsc_Mode2_Count<3)
            {
                R842_Atsc_Mode2_Count++;
            }

            if(R842_Atsc_Mode2_Count>=2)
            {
                R842_Atsc_Mode = 2;
                R842_Atsc_Mode1_Count = 0; //clear mode1 count
            }
        }
        else if(filter_gain>8)  //mode1 (slow_narrow)
        {
            if(R842_Atsc_Mode1_Count<3)
            {
                R842_Atsc_Mode1_Count++;
            }

            if(R842_Atsc_Mode1_Count>=2)
            {
                R842_Atsc_Mode = 1;
                R842_Atsc_Mode2_Count = 0; //clear mode2 count
            }
        }
    }  //end if(freq_KHz)

    //set parameters by mode
    if(R842_Atsc_Mode != R842_Atsc_Mode_Pre)
    {
        if(R842_Atsc_Mode==0) //fast
        {
            // HPF = R842_Atsc_HPF R24[3:0]
            R842_Array[24] = (R842_Array[24] & 0xF0) | R842_Atsc_HPF;

            // LPF = R842_Atsc_LPF R22[4:0]
            R842_Array[22] = (R842_Array[22] & 0xE0) | R842_Atsc_LPF;

            // LNA VTL/H = 0.84/1.34
            R842_Array[38] = (R842_Array[38] & 0x00) | 0x5A;

            // RF VTL/H = 0.94/1.44
            R842_Array[39] = (R842_Array[39] & 0x00) | 0x6B;

            // MIXER VTH = 1.24
            R842_Array[41] = (R842_Array[41] & 0xF0) | 0x09;

            // MIXER VTL = 0.74
            R842_Array[43] = (R842_Array[43] & 0xF0) | 0x04;

            // Mixer Gain Limt = 8
            R842_Array[20] = (R842_Array[20] & 0x9F) | 0x20;
            //R842_Array[20] = (R842_Array[20] & 0x9F) | 0x00;

            // NRB TOP = 6
            R842_Array[40] = (R842_Array[40] & 0x0F) | ((15 - 6)<<4);
            ...
            R842_Array[23] = (R842_Array[23] & 0xFE) | 0x01;

			 //AGC clk = 512Hz
			 R842_Array[47] = (R842_Array[47] & 0xF3) | 0x04;  //R47[3:2]=01 (512Hz)
		}
		else if(R842_Atsc_Mode==1)  //slow_narrow
		{
			/*if(R842_Atsc_Mode_Pre==2)  //slow_wide -> slow_narrow
			{
				if(mixer_gain>=6)
				{				  
					 //Mixer off (R12[6]=0)
					 R842_I2C.RegAddr = 12;
					 R842_Array[12] = (R842_Array[12] & 0xBF) | 0x00;
					 R842_I2C.Data = R842_Array[12];
					 I2C_Write(&R842_I2C);

					 //Mixer gain=7
					 R842_I2C.RegAddr = 41;
					 R842_Array[41] = (R842_Array[41] & 0xF0) | 7;  
					 R842_I2C.Data = R842_Array[41];
					 I2C_Write(&R842_I2C);						  
				}			
			}*/
			...
			if((Freq_KHz>500000) && (Freq_KHz<600000))
			{
						...
						if(R842_Atsc_HPF>=13)
						{
							R842_Array[24] = (R842_Array[24] & 0xF0) | 0x0F;  //HPF set 15
						}
						else
						{	
							R842_Array[24] = (R842_Array[24] & 0xF0) | (R842_Atsc_HPF+2);  //HPF set R842_Atsc_HPF+2
						}
			 
						 // LPF = R842_Atsc_LPF R22[4:0]
						if(R842_Atsc_LPF<=3)
						{
							R842_Array[22] = (R842_Array[22] & 0xE0) | 0x00; //LPF set 0
						}
						else
						{
							R842_Array[22] = (R842_Array[22] & 0xE0) | (R842_Atsc_LPF-3); //LPF set R842_Atsc_LPF-3
						}			
			}  //end if(freq_KHz)
            else
			{
            ...
            // Mixer Gain Limt = 8
            R842_Array[20] = (R842_Array[20] & 0x9F) | 0x20;  //8
            //R842_Array[20] = (R842_Array[20] & 0x9F) | 0x00;  //6
            ...
            R842_Array[23] = (R842_Array[23] & 0xFE) | 0x01;
            /*
            //LNA_RF Discharge Mode = both(low)
            R842_Array[45] = (R842_Array[45] & 0xFC) | 0x02;   //10
            R842_Array[9] = (R842_Array[9] & 0xBF) | 0x00;     //0

			 //LNA_RF_dis current = 1/3
			 R842_Array[23] = (R842_Array[23] & 0xFE) | 0x00;  
*/
			 //AGC clk = 128Hz
			 R842_Array[47] = (R842_Array[47] & 0xF3) | 0x0C;  //R47[3:2]=11 (128Hz)	
		}
		else  //slow_wide
		{
			/*if(R842_Atsc_Mode_Pre==1)  //slow_narrow -> slow_wide 
			{
				if(mixer_gain>=8)
				{				  
					 //Mixer off (R12[6]=0)
            ...
            else
			{
			 ...
			 // LNA VTL/H = 0.64/1.44	 
			 R842_Array[38] = (R842_Array[38] & 0x00) | 0x4C;  

            // RF VTL/H = 0.74/1.44
            R842_Array[39] = (R842_Array[39] & 0x00) | 0x4B;

            // MIXER VTH = 1.34
            R842_Array[41] = (R842_Array[41] & 0xF0) | 0x0A;

            // MIXER VTL = 0.64
            R842_Array[43] = (R842_Array[43] & 0xF0) | 0x03;
        ...
        R842_I2C.RegAddr = 43;
        R842_I2C.Data = R842_Array[43];
        I2C_Write(&R842_I2C);

        R842_I2C.RegAddr = 45;
        R842_I2C.Data = R842_Array[45];
        I2C_Write(&R842_I2C);
        ...
        R842_I2C.RegAddr = 20;
        R842_I2C.Data = R842_Array[20];  //max_gain_limit, move to last I2C_write
        I2C_Write(&R842_I2C);

        //Auto Mixer AGC (R12[6]=1)
        R842_I2C.RegAddr = 12;
        R842_Array[12] = (R842_Array[12] & 0xBF) | 0x40;
        R842_I2C.Data = R842_Array[12];
        I2C_Write(&R842_I2C);

    } //end if(mode!=mode_pre)

    R842_Atsc_Mode_Pre = R842_Atsc_Mode;
...
//---------------TF check Function-------------//
//---------------------------------------------//
// Property: check TF_low inductor             //
// Input: TF_low_Type                          //
//        0: 270nH                             //
//        1: Bead                              //
// Output: 0: Check Pass                       //
//         1: Check Fail                       //
//---------------------------------------------//
UINT8 R842_TF_Low_Check(UINT8 TF_Low_Type)
{
    UINT8 R842_TF_TEST_Array[R842_REG_NUM];

    //TF_low Bead detect
    UINT8 R842_TF_Bead_LNACal_Array[R842_REG_NUM] = {   //24M
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  //read only
    0x0C, 0xFA, 0x10, 0x90, 0xB0, 0xC1, 0x20, 0xBF,
    0x00, 0x57, 0x86, 0x30, 0x60, 0x00, 0x7F, 0x69,
    0x00, 0x34, 0x89, 0x55, 0x55, 0x58, 0xBE, 0x5C,
    0x21, 0x3F, 0x52, 0xA3, 0xE6, 0x38, 0x38, 0x08,
    0x63, 0x00, 0xC2, 0x45, 0x55, 0xFB, 0xAC, 0x05 };

    //TF_low 270nH detect
    UINT8 R842_TF_72M_RFCal_Array[R842_REG_NUM] = {   //24M
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  //read only
    0x0A, 0xF2, 0x70, 0x90, 0xB0, 0xDF, 0x20, 0x8F,     //R8=0x0A(RF buf PW=off), R15=0x8F(TF cal=off, -6dB=off)
    0x00, 0x42, 0x86, 0x30, 0x60, 0x00, 0x19, 0x69,
    0x00, 0xB4, 0x89, 0x55, 0x55, 0x58, 0x3E, 0x40, 	   //R25=B4(Ring RfBuf out=Enable)
    0x21, 0x1E, 0x52, 0xA3, 0xE6, 0x38, 0x38, 0x08,
    0x63, 0x00, 0xC2, 0x45, 0x55, 0xFB, 0xAC, 0x05 };

    //LNACal: R8=0x08;R15=0xBF; R25=0x34;
    //51.4 MHz:R26=0x8F, R27=0x33, R28=0x33, R34=0xEF, R35=0xAB
    //116.6MHz:R26=0x91, R27=0x11, R28=0x11, R34=0x71, R35=0x22

    I2C_TYPE  R842_I2C_UPDATE;
    I2C_LEN_TYPE R842_I2C_Len_UPDATE;

    UINT8 ADC_Read_Value_RFCal_A = 0;
    UINT8 ADC_Read_Value_RFCal_B = 0;
    UINT8 ADC_Read_Value_RFCal_C = 0;
    UINT8 ADC_Read_Value_LNACal_A = 0;
    UINT8 ADC_Read_Value_LNACal_B = 0;
    UINT8 ADC_Read_Value_LNACal_C = 0;
    UINT8 LPF_Count = 0;
    UINT8 result1 = 0;  //0:pass
    UINT8 result2 = 0;
    UINT8 result = 0;
    int i;

    if(TF_Low_Type==0)  //270nH
    {
        //write 72MHz(A) RFCal setting
        for(i=8; i<R842_REG_NUM; i++)
        {
        R842_I2C_Len_UPDATE.Data[i] = R842_TF_72M_RFCal_Array[i];
        R842_TF_TEST_Array[i] = R842_TF_72M_RFCal_Array[i];
    }

    R842_I2C_Len_UPDATE.RegAddr = 0;
    R842_I2C_Len_UPDATE.Len = R842_REG_NUM;
    I2C_Write_Len(&R842_I2C_Len_UPDATE);

    //filter gain training
    for(LPF_Count=0; LPF_Count < 16; LPF_Count ++)
    {
        R842_I2C_UPDATE.RegAddr = 41;
        R842_TF_TEST_Array[41] = (R842_TF_TEST_Array[41] & 0x0F) | (LPF_Count<<4);
        R842_I2C_UPDATE.Data = R842_TF_TEST_Array[41];
        I2C_Write(&R842_I2C_UPDATE);

        R842_Delay_MS(3);

        R842_Muti_Read(&ADC_Read_Value_RFCal_A);

        if(ADC_Read_Value_RFCal_A > 30)
            break;
    }

    //change to 72MHz LNA Cal setting
    R842_I2C_UPDATE.RegAddr = 8;
    R842_I2C_UPDATE.Data = 0x08;  //RF buf PW=on
    R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr] = R842_I2C_UPDATE.Data;
    I2C_Write(&R842_I2C_UPDATE);

    R842_I2C_UPDATE.RegAddr = 15;
    R842_I2C_UPDATE.Data = 0xBF;  //TF_cal=on, -6dB=on
    R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr] = R842_I2C_UPDATE.Data;
    I2C_Write(&R842_I2C_UPDATE);

    R842_I2C_UPDATE.RegAddr = 25;
    R842_I2C_UPDATE.Data = 0x34;  //Ring RfBuf out=disable
    R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr] = R842_I2C_UPDATE.Data;
    I2C_Write(&R842_I2C_UPDATE);

    R842_Delay_MS(3);

    R842_Muti_Read(&ADC_Read_Value_LNACal_A);

    if((ADC_Read_Value_LNACal_A - ADC_Read_Value_RFCal_A) >= 10)  //pass
    {
        result1 = 0;
//			SetDlgItemText(IDC_EDIT1, "Pass");
    }
    else
    {
    result1 = 1;
    //SetDlgItemText(IDC_EDIT1, "Fail");
    }

    //Try 116.6MHz(B) LNACal
    //116.6MHz:R26=0x91, R27=0x11, R28=0x11, R34=0x71, R35=0x22
    R842_I2C_UPDATE.RegAddr = 26;
    R842_I2C_UPDATE.Data = 0x91;
    R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr] = R842_I2C_UPDATE.Data;
    I2C_Write(&R842_I2C_UPDATE);

    R842_I2C_UPDATE.RegAddr = 27;
    R842_I2C_UPDATE.Data = 0x11;
    R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr] = R842_I2C_UPDATE.Data;
    I2C_Write(&R842_I2C_UPDATE);

    R842_I2C_UPDATE.RegAddr = 28;
    R842_I2C_UPDATE.Data = 0x11;
    R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr] = R842_I2C_UPDATE.Data;
    I2C_Write(&R842_I2C_UPDATE);

    R842_I2C_UPDATE.RegAddr = 34;
    R842_I2C_UPDATE.Data = 0x71;
    R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr] = R842_I2C_UPDATE.Data;
    I2C_Write(&R842_I2C_UPDATE);

    R842_I2C_UPDATE.RegAddr = 35;
    R842_I2C_UPDATE.Data = 0x22;
    R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr] = R842_I2C_UPDATE.Data;
    I2C_Write(&R842_I2C_UPDATE);

    R842_Delay_MS(3);

    R842_Muti_Read(&ADC_Read_Value_LNACal_B);

    //Try 116.6MHz(B) RFCal
    R842_I2C_UPDATE.RegAddr = 8;
    R842_I2C_UPDATE.Data = 0x0A;  //RF buf PW=off
    R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr] = R842_I2C_UPDATE.Data;
    I2C_Write(&R842_I2C_UPDATE);

    R842_I2C_UPDATE.RegAddr = 15;
    R842_I2C_UPDATE.Data = 0x8F;  //TF_cal=off, -6dB=off
    R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr] = R842_I2C_UPDATE.Data;
    I2C_Write(&R842_I2C_UPDATE);

    R842_I2C_UPDATE.RegAddr = 25;
    R842_I2C_UPDATE.Data = 0xB4;  //Ring RfBuf out=enable
    R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr] = R842_I2C_UPDATE.Data;
    I2C_Write(&R842_I2C_UPDATE);

    R842_Delay_MS(3);

    R842_Muti_Read(&ADC_Read_Value_RFCal_B);

    //Try 51.4MHz(C) RFCal
    //51.4 MHz:R26=0x8F, R27=0x33, R28=0x33, R34=0xEF, R35=0xAB
    R842_I2C_UPDATE.RegAddr = 26;
    R842_I2C_UPDATE.Data = 0x8F;
    R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr] = R842_I2C_UPDATE.Data;
    I2C_Write(&R842_I2C_UPDATE);

    R842_I2C_UPDATE.RegAddr = 27;
    R842_I2C_UPDATE.Data = 0x33;
    R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr] = R842_I2C_UPDATE.Data;
    I2C_Write(&R842_I2C_UPDATE);
    ...
    R842_I2C_UPDATE.RegAddr = 34;
    R842_I2C_UPDATE.Data = 0xEF;
    R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr] = R842_I2C_UPDATE.Data;
    I2C_Write(&R842_I2C_UPDATE);

    R842_I2C_UPDATE.RegAddr = 35;
    R842_I2C_UPDATE.Data = 0xAB;
    R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr] = R842_I2C_UPDATE.Data;
    I2C_Write(&R842_I2C_UPDATE);

    R842_Delay_MS(3);

    R842_Muti_Read(&ADC_Read_Value_RFCal_C);

    //Try 51.4MHz(C) LNACal
    R842_I2C_UPDATE.RegAddr = 8;
    R842_I2C_UPDATE.Data = 0x08;  //RF buf PW=on
    R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr] = R842_I2C_UPDATE.Data;
    I2C_Write(&R842_I2C_UPDATE);
    ...
    R842_Muti_Read(&ADC_Read_Value_LNACal_C);

    //LNACal ADC value after cal
    ADC_Read_Value_LNACal_B = ADC_Read_Value_LNACal_B + ADC_Read_Value_RFCal_A - ADC_Read_Value_RFCal_B;
    ADC_Read_Value_LNACal_C = ADC_Read_Value_LNACal_C + ADC_Read_Value_RFCal_A - ADC_Read_Value_RFCal_C;

    if((ADC_Read_Value_LNACal_A>=(ADC_Read_Value_LNACal_B-5)) && (ADC_Read_Value_LNACal_A>=(ADC_Read_Value_LNACal_C-5)))
    {
        result2 = 0;
        //SetDlgItemText(IDC_EDIT2, "Pass");
    }
    else
    {
        result2 = 1;
        //SetDlgItemText(IDC_EDIT2, "Fail");
    }

    if((result1==1) || (result2==1))
    {
        result = 1;
        //MessageBox(_T("TF_low Fail!"));
    }
    else
    {
        result = 0;
    ...
    } //end of Tf_low detect
    else  //Bead detect
    {
    //write Bead LNACal setting
        for(i=8; i<R842_REG_NUM; i++)
        {
            R842_I2C_Len_UPDATE.Data[i] = R842_TF_Bead_LNACal_Array[i];
            R842_TF_TEST_Array[i] = R842_TF_Bead_LNACal_Array[i];
        }
        ...
        //Set LPF Gain = 0
        R842_I2C_UPDATE.RegAddr = 41;
        R842_TF_TEST_Array[41] = (R842_TF_TEST_Array[41] & 0x0F) | (0<<4);
        R842_I2C_UPDATE.Data = R842_TF_TEST_Array[41];
        I2C_Write(&R842_I2C_UPDATE);

        //Set LNA TF=(0,0)
        R842_I2C_UPDATE.RegAddr = 14;
        R842_TF_TEST_Array[14] = (R842_TF_TEST_Array[14] & 0x80);
        R842_I2C_UPDATE.Data = R842_TF_TEST_Array[14];
        I2C_Write(&R842_I2C_UPDATE);
        ...
        //Set LNA TF=32
        R842_I2C_UPDATE.RegAddr = 14;
        R842_TF_TEST_Array[14] = (R842_TF_TEST_Array[14] & 0x80) | 32 ;
        R842_I2C_UPDATE.Data = R842_TF_TEST_Array[14];
        I2C_Write(&R842_I2C_UPDATE);
        ...
        //if ADC_Read=0, TF_Check_Fail
        if((ADC_Read_Value_LNACal_A==0) && (ADC_Read_Value_LNACal_B==0))
        {
             result2 = 1;
         //SetDlgItemText(IDC_EDIT2, "Fail!ADC=0");
        }
        else
        {
             result2 = 0;
         //SetDlgItemText(IDC_EDIT2, "Pass!ADC not 0");
        }

        if(ADC_Read_Value_LNACal_A > ADC_Read_Value_LNACal_B)  //Bead
        {
            result1 = 0;
        //SetDlgItemText(IDC_EDIT1, "Pass");
        }
...
//---------------------------------------------//
// Property: check TF_mid inductor             //
// Input: TF_mid_Type                          //
//        0: 39nH                              //
//        1: 68nH                              //
// Output: 0: Check Pass                       //
//         1: Check Fail                       //
//---------------------------------------------//
UINT8 R842_TF_Mid_Check(UINT8 TF_Mid_Type)
{
    UINT8 R842_TF_TEST_Array[R842_REG_NUM];

    //TF_mid 39nH detect
    UINT8 R842_TF_272M_RFCal_Array[R842_REG_NUM] = {   //24M
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  //read only
    0x0A, 0xF2, 0x70, 0x90, 0xB0, 0xDF, 0x0E, 0x8E,
    0x00, 0x16, 0x86, 0x30, 0x60, 0x00, 0x19, 0x69,
    0x00, 0xB4, 0x88, 0xAA, 0x2A, 0x58, 0x3E, 0x40,
    0x21, 0x1E, 0x51, 0x51, 0xE6, 0x38, 0x38, 0x08,
    0x63, 0x00, 0xC2, 0x45, 0x55, 0xFB, 0xAC, 0x05 };

    //LNACal: R8=0x08;R15=0xBE; R25=0x34;
    //153.6MHz:R26=0x89, R27=0xDE, R28=0xDD, R34=0xB0, R35=0xDA
    //408.0MHz:R26=0x8D, R27=0x55, R28=0xD5, R34=0x11, R35=0xD1

    //TF_mid 68nH detect
    UINT8 R842_TF_192M_RFCal_Array[R842_REG_NUM] = {   //24M
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  //read only
    0x0A, 0xF2, 0x70, 0x90, 0xB0, 0xDF, 0x0F, 0x8E,
    0x00, 0x17, 0x86, 0x30, 0x60, 0x00, 0x19, 0x69,
    0x00, 0xB4, 0x8D, 0xAA, 0xAA, 0x58, 0x3E, 0x40,
    0x21, 0x1E, 0x90, 0x1A, 0xE6, 0x38, 0x38, 0x08,
    0x63, 0x00, 0xC2, 0x45, 0x55, 0xFB, 0xAC, 0x05 };

    //LNACal: R8=0x08;R15=0xBE; R25=0x34;
    //108MHz:R26=0x8F, R27=0x55, R28=0x55, R34=0x92, R35=0xA3
    //288MHz:R26=0x88, R27=0x55, R28=0xD5, R34=0xD2, R35=0xD1
    ...
    if(TF_Mid_Type==0)  //TF_mid=39nH
    {
    //write 272MHz(A) RFCal setting
        for(i=8; i<R842_REG_NUM; i++)
        {
            R842_I2C_Len_UPDATE.Data[i] = R842_TF_272M_RFCal_Array[i];
            R842_TF_TEST_Array[i] = R842_TF_272M_RFCal_Array[i];
        }
    ...
    //change to 272MHz LNA Cal setting
        R842_I2C_UPDATE.RegAddr = 8;
        R842_I2C_UPDATE.Data = 0x08;  //RF buf PW=on
        R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr] = R842_I2C_UPDATE.Data;
        I2C_Write(&R842_I2C_UPDATE);
        ...
        R842_I2C_UPDATE.RegAddr = 15;
        R842_I2C_UPDATE.Data = 0xBE;  //TF_cal=on, -6dB=on
        R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr] = R842_I2C_UPDATE.Data;
        I2C_Write(&R842_I2C_UPDATE);
        ...
        if((ADC_Read_Value_LNACal_A - ADC_Read_Value_RFCal_A) >= 0)  //pass
        {
        result1 = 0;
        //SetDlgItemText(IDC_EDIT3, "Pass");
        }
        else
        {
        result1 = 1;
        //SetDlgItemText(IDC_EDIT3, "Fail");
        }

        //Try 408MHz(B) LNACal
        //408.0MHz:R26=0x8D, R27=0x55, R28=0xD5, R34=0x11, R35=0xD1
        R842_I2C_UPDATE.RegAddr = 26;
        R842_I2C_UPDATE.Data = 0x8D;
        R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr] = R842_I2C_UPDATE.Data;
        I2C_Write(&R842_I2C_UPDATE);

        R842_I2C_UPDATE.RegAddr = 27;
        R842_I2C_UPDATE.Data = 0x55;
        R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr] = R842_I2C_UPDATE.Data;
        I2C_Write(&R842_I2C_UPDATE);

        R842_I2C_UPDATE.RegAddr = 28;
        R842_I2C_UPDATE.Data = 0xD5;
        R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr] = R842_I2C_UPDATE.Data;
        I2C_Write(&R842_I2C_UPDATE);
        ...
        R842_I2C_UPDATE.RegAddr = 15;
        R842_I2C_UPDATE.Data = 0x8E;  //TF_cal=off, -6dB=off
        R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr] = R842_I2C_UPDATE.Data;
        I2C_Write(&R842_I2C_UPDATE);
        ...
        //Try 153.6MHz(C) RFCal
        //153.6MHz:R26=0x89, R27=0xDE, R28=0xDD, R34=0xB0, R35=0xDA
        R842_I2C_UPDATE.RegAddr = 26;
        R842_I2C_UPDATE.Data = 0x89;
        R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr] = R842_I2C_UPDATE.Data;
        I2C_Write(&R842_I2C_UPDATE);

        R842_I2C_UPDATE.RegAddr = 27;
        R842_I2C_UPDATE.Data = 0xDE;
        R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr] = R842_I2C_UPDATE.Data;
        I2C_Write(&R842_I2C_UPDATE);

        R842_I2C_UPDATE.RegAddr = 28;
        R842_I2C_UPDATE.Data = 0xDD;
        R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr] = R842_I2C_UPDATE.Data;
        I2C_Write(&R842_I2C_UPDATE);

        R842_I2C_UPDATE.RegAddr = 34;
        R842_I2C_UPDATE.Data = 0xB0;
        R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr] = R842_I2C_UPDATE.Data;
        I2C_Write(&R842_I2C_UPDATE);

        R842_I2C_UPDATE.RegAddr = 35;
        R842_I2C_UPDATE.Data = 0xDA;
        R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr] = R842_I2C_UPDATE.Data;
        I2C_Write(&R842_I2C_UPDATE);
        ...
        {
            result2 = 0;
            //SetDlgItemText(IDC_EDIT4, "Pass");
        }
        else
        {
            result2 = 1;
            //SetDlgItemText(IDC_EDIT4, "Fail");
        }
    ...
    else //TF_mid=68nH
    {
        ...
        //write 192MHz(A) RFCal setting
        for(i=8; i<R842_REG_NUM; i++)
        {
            R842_I2C_Len_UPDATE.Data[i] = R842_TF_192M_RFCal_Array[i];
            R842_TF_TEST_Array[i] = R842_TF_192M_RFCal_Array[i];
        }
        ...
        //change to 192MHz LNA Cal setting
        R842_I2C_UPDATE.RegAddr = 8;
        R842_I2C_UPDATE.Data = 0x08;  //RF buf PW=on
        R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr] = R842_I2C_UPDATE.Data;
        I2C_Write(&R842_I2C_UPDATE);
        ...
        if((ADC_Read_Value_LNACal_A - ADC_Read_Value_RFCal_A) >= 6)  //pass
        {
            result1 = 0;
            //SetDlgItemText(IDC_EDIT3, "Pass");
        }
        ...
        R842_I2C_UPDATE.RegAddr = 34;
        R842_I2C_UPDATE.Data = 0x92;
        R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr] = R842_I2C_UPDATE.Data;
        I2C_Write(&R842_I2C_UPDATE);

        R842_I2C_UPDATE.RegAddr = 35;
        R842_I2C_UPDATE.Data = 0xA3;
        R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr] = R842_I2C_UPDATE.Data;
        I2C_Write(&R842_I2C_UPDATE);
    ...
    } //end of TF_mid detect

    return result;
}
//---------------------------------------------//
// Property: check LPF TF inductor             //
// Output: 0: Check Pass                       //
//         1: Check Fail                       //
//---------------------------------------------//
UINT8 R842_TF_LPF_Check(void)
{
    UINT8 R842_TF_TEST_Array[R842_REG_NUM];

    //LPF TF 180nH detect
    UINT8 R842_TF_304M_LNACal_Array[R842_REG_NUM] = {   //24M
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  //read only
    0x08, 0xF2, 0x70, 0x90, 0xB0, 0xDF, 0x07, 0xFE,
    0x00, 0x17, 0x86, 0x30, 0x60, 0x00, 0x19, 0x69,
    0x00, 0x34, 0x89, 0x00, 0x80, 0x58, 0x3E, 0x40,
    0x21, 0x1E, 0x53, 0x91, 0xE6, 0x38, 0x38, 0x38,
    0x63, 0x00, 0xC2, 0x45, 0x55, 0xFB, 0xAC, 0x05 };
    ...
    UINT8 min_count = 0;
    UINT8 ADC_Read_Value_LNACal_A = 0;
    UINT8 ADC_Read_Value_LNACal_LPF_TF[16];
    UINT8 result = 0;
    int i;

    for(i=8; i<R842_REG_NUM; i++)
    {
        R842_I2C_Len_UPDATE.Data[i] = R842_TF_304M_LNACal_Array[i];
        R842_TF_TEST_Array[i] = R842_TF_304M_LNACal_Array[i];
    }
    ...
    for(LPF_Count=0; LPF_Count < 16; LPF_Count ++)
    {
        ...
        if(ADC_Read_Value_LNACal_A > 50)
            break;
    }

    //change LPF_notch from 0~15
    ADC_Read_Value_LNACal_A = 63;  //initial at max. ADC value
    min_count = 0;

	for(LPF_Count=0; LPF_Count < 16; LPF_Count ++)  
	{
		R842_I2C_UPDATE.RegAddr = 16;
		R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr] = (R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr] & 0x0F) | (LPF_Count<<4); 
		R842_I2C_UPDATE.Data = R842_TF_TEST_Array[R842_I2C_UPDATE.RegAddr];		
		I2C_Write(&R842_I2C_UPDATE);

		R842_Delay_MS(3); 
			
		R842_Muti_Read(&ADC_Read_Value_LNACal_LPF_TF[LPF_Count]);

		//serach ADC min value, should notch in index 7 or 8
		if(ADC_Read_Value_LNACal_LPF_TF[LPF_Count] < ADC_Read_Value_LNACal_A)
		{
			ADC_Read_Value_LNACal_A = ADC_Read_Value_LNACal_LPF_TF[LPF_Count]; //min value
			min_count = LPF_Count; 
		}
	}

	if((min_count==7) || (min_count==8))
	{
		result = 0;
		//SetDlgItemText(IDC_EDIT6, "Pass");
	}
	else
	{
		result = 1;
		//SetDlgItemText(IDC_EDIT6, "Fail");
	}

	return result;
}

R842_BOOL UserWrittenI2CRead(U32 AddrSize, U8* pAddr,U32 ReadLen, U8* pData);
R842_BOOL UserWrittenI2CWrite (U32 AddrSize, U8* pAddr,U32 WriteLen, U8* pData);
...
{
    UINT8 i;
    R842_BOOL bRet = UserWrittenI2CRead(0, NULL, I2C_Info->Len, I2C_Info->Data);

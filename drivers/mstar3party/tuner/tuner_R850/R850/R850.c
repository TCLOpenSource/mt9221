...
#define R850_Delay_MS(R850_Tuner_Num, x_ms)     mdelay(x_ms)//;(x_ms)
//==========Device Address============//
...
//=======NA defined========
//0.reference(E5071C(4G),E5071C(20G),E5062A,E5061B)    (default)
/////1.Hp 8753E,R&S ZvL
/////2.E5071C(8G)
//3.
//4.E5071C(6.5G)
//5.DEVISER
//6.R&S ZNC3
UINT8 R850_NA_TYPE = 2;  //(0 is default)
...
UINT8* pR850_Xtal_Pwr = NULL;
UINT8* pR850_Xtal_Pwr_tmp = NULL;
...
UINT8* pR850_Array  = NULL;
R850_SectType R850_IMR_Data[MAX_TUNER_NUM][R850_IMR_POINT_NUM] = {0};
I2C_TYPE  R850_I2C;
...
UINT8*  pR850_Initial_done_flag = NULL;
UINT8*  pR850_IMR_done_flag = NULL;
UINT8  R850_Bandwidth = 0;   //8M
UINT8  R850_Lpf_Lsb = 0;
UINT8*  pR850_IMR_Cal_Result = NULL;
UINT8*  pR850_Fil_Cal_flag = NULL;
UINT8*  pR850_Fil_Cal_BW = NULL;
UINT8*  pR850_Fil_Cal_code = NULL;
UINT8*  pR850_Fil_Cal_LpfLsb = NULL;
...
UINT8*  pR850_Atsc_Mode = NULL;
UINT8*  pR850_Atsc_Mode_Pre = NULL;
UINT8*  pR850_Atsc_Mode1_Count = NULL;
UINT8*  pR850_Atsc_Mode2_Count = NULL;
UINT8*  pR850_Atsc_LPF = NULL;
UINT8*  pR850_Atsc_HPF = NULL;
...
BOOL R850_Sys_Sel(R850_Sys_Info_Type *pR850_Sys_Info,R850_Standard_Type R850_Standard);
...
BOOL I2C_Write_Len(R850_TUNER_NUM R850_Tuner_Num,I2C_LEN_TYPE *I2C_Info);
BOOL I2C_Read_Len(R850_TUNER_NUM R850_Tuner_Num,I2C_LEN_TYPE *I2C_Info);
BOOL I2C_Write(R850_TUNER_NUM R850_Tuner_Num,I2C_TYPE *I2C_Info);
UINT8 Rafael_Convert(UINT8 InvertNum);

static UINT16 R850_Lna_Acc_Gain_NORMAL_TYPE[4][32] =
{
...
static INT8 Lna_Acc_Gain_offset_NORMAL_TYPE[86]=
{  6,  4, 4, 3, 3, 4, 3, 2, 2, 1,//45~145
...
static UINT16 R850_Lna_Acc_Gain_ISDBT[4][32] =
{
...
static INT8 Lna_Acc_Gain_offset_ISDBT[86]={-7, -9, -9, -10, -9, -9, -10, -11, -11, -12,	//45~145
...
static void R850_SET_REG_Array(UINT8 u8TunerIndex, UINT8 reg, UINT8 Mask,UINT8 Value)
{
    UINT8 u8tmp;
...
static UINT8 R850_GET_REG_Array(UINT8 u8TunerIndex, UINT8 reg)
{
...
R850_SysFreq_Info_Type R850_SysFreq_NrbDetOn_Sel(R850_Standard_Type R850_Standard,UINT32 RF_freq)
{
    ...
    case R850_DVB_T2_1_7M_IF_5M:
        ...
        else if ((RF_freq>686000) && (RF_freq<=694000))
        {
            //LNA
            R850_SysFreq_Info.LNA_TOP=4;                    //R38[2:0] ["7~0" (0~7) ; input: "7~0"]
        ...
        //over write, for ETSI EN 303 340 DVB-T/T2 LTE test
        if ((RF_freq>686000) && (RF_freq<=694000))
        {
            R850_SysFreq_Info.LNA_RF_DIS_CURR=0;            //R13[5]        ["1/3 dis current" (0), "normal" (1)]
        }
        break;
...
R850_ErrCode R850_Init(R850_TUNER_NUM R850_Tuner_Num, R850_Standard_Type R850_Standard)
{
    ...
    R850_Sys_Sel (&R850_Sys_Init, R850_Standard);    //Get R850_Mixer_Mode by different standard
...
#endif
    if(*(pR850_Initial_done_flag + R850_Tuner_Num)==FALSE)
    {
          //reset filter cal. data
          for (i=0; i<R850_STD_SIZE; i++)
          {
                *(pR850_Fil_Cal_flag + R850_Tuner_Num *R850_STD_SIZE +i) = FALSE;
                *(pR850_Fil_Cal_code + R850_Tuner_Num *R850_STD_SIZE +i) = 0;
                *(pR850_Fil_Cal_BW + R850_Tuner_Num *R850_STD_SIZE +i) = 0x00;
                *(pR850_Fil_Cal_LpfLsb + R850_Tuner_Num *R850_STD_SIZE +i) = 0x00;
          }

          *(pR850_IMR_Cal_Result + R850_Tuner_Num) = 0;

          if(*(pR850_IMR_done_flag + R850_Tuner_Num)==FALSE)
          {
            ...
            *(pR850_IMR_done_flag + R850_Tuner_Num) = TRUE;
...
//other          => Xtal PW = Highest
          ...
          //do Xtal check
#if(R850_SHARE_XTAL_MASTER==TRUE)
         *(pR850_Xtal_Pwr + R850_Tuner_Num) = XTAL_HIGHEST;
//#elif(R850_SHARE_XTAL_SLAVE==TRUE)
//          R850_Xtal_Pwr[R850_Tuner_Num] = XTAL_LOWEST;
          ...
          if(*(pR850_Xtal_Pwr_tmp + R850_Tuner_Num)==XTAL_HIGHEST)
              *(pR850_Xtal_Pwr + R850_Tuner_Num) = XTAL_HIGHEST;
          else
              *(pR850_Xtal_Pwr + R850_Tuner_Num) = *(pR850_Xtal_Pwr_tmp + R850_Tuner_Num)+1;

#endif

          *(pR850_Initial_done_flag + R850_Tuner_Num) = TRUE;
    ...
    *(pR850_pre_standard + R850_Tuner_Num) = R850_STD_SIZE;
...
R850_ErrCode R850_InitReg(R850_TUNER_NUM R850_Tuner_Num)//ok
{
    ...
    if(R850_Xtal == 24000)
    {
        for(InitArrayCunt = 8; InitArrayCunt<R850_REG_NUM; InitArrayCunt ++)
        {
            ...
            R850_SET_REG_Array(R850_Tuner_Num, InitArrayCunt, 0, R850_iniArray[0][InitArrayCunt]);
        }
    }
    else if(R850_Xtal == 16000)
    {
        for(InitArrayCunt = 8; InitArrayCunt<R850_REG_NUM; InitArrayCunt ++)
        {
            ...
            R850_SET_REG_Array(R850_Tuner_Num, InitArrayCunt, 0, R850_iniArray[1][InitArrayCunt]);
        }
    }
    else if(R850_Xtal == 27000)
    {
        for(InitArrayCunt = 8; InitArrayCunt<R850_REG_NUM; InitArrayCunt ++)
        {
            ...
            R850_SET_REG_Array(R850_Tuner_Num, InitArrayCunt, 0, R850_iniArray[2][InitArrayCunt]);
        }
    }
    else
    {
    ...
    //Clock Out Enable
    R850_I2C_Len.Data[34] = (R850_GET_REG_Array(R850_Tuner_Num, 34) & 0xCB) ;
    R850_SET_REG_Array(R850_Tuner_Num, 34, 0xCB, 0);
#endif
/*
#if(R850_SHARE_XTAL_SLAVE==TRUE)
    //gm off R34[7:6]=11
     R850_I2C_Len.Data[34] = (R850_Array[R850_Tuner_Num][34] | 0xC0) ;
     R850_Array[R850_Tuner_Num][34] = (R850_Array[R850_Tuner_Num][34] | 0xC0);
#endif
*/
...
R850_ErrCode R850_Xtal_Check(R850_TUNER_NUM R850_Tuner_Num)
{
    ...
    if(R850_Chip == R850_MP)  // R47[1:0] = 0
        R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFC, 0);
    else // R47[1] = 0
        R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFD, 0);
    R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, 47);
...
*/
    ...
    {
        R850_SET_REG_Array(R850_Tuner_Num, 27, 0x80, 18);              //Ni=18    => R27
        R850_SET_REG_Array(R850_Tuner_Num, 30, 0xFC, 0);                //Si=0    => R30[1:0]
        R850_SET_REG_Array(R850_Tuner_Num, 29, 0x0F, 0<<4);                //SDM=0000 0000 0000 00000    => R29[7:4]
        R850_SET_REG_Array(R850_Tuner_Num, 30, 0xE3, 0x08);          //div 8 => R30[4:2]
        R850_SET_REG_Array(R850_Tuner_Num, 34, 0x27, 0x00 | 0x00);  //gm*2(24); Xtal div1 => R34[7:6] , R34[0]
    ...
    {
        R850_SET_REG_Array(R850_Tuner_Num, 27, 0x80, 28);             //Ni=28 => R27
        R850_SET_REG_Array(R850_Tuner_Num, 30, 0xFC, 2);                //Si=2  => R30[1:0]
        R850_SET_REG_Array(R850_Tuner_Num, 29, 0x0F, 8<<4);                //SDM=0000 0000 0000 00000    => R29[7:4]
        R850_SET_REG_Array(R850_Tuner_Num, 30, 0xE3, 0x08);            //div 8 => R30[4:2]
        R850_SET_REG_Array(R850_Tuner_Num, 34, 0x3E, 0x80 | 0x00); //gm(16); Xtal div1 => R34[7:6] , R34[0]
    ...
    else if (R850_Xtal == 27000) //LO=509.9941MHz, VCO=4079.953MHz
    {
        R850_SET_REG_Array(R850_Tuner_Num, 27, 0x80, 28);             //Ni=28 => R27
        R850_SET_REG_Array(R850_Tuner_Num, 30, 0xFC, 2);                //Si=2  => R30[1:0]
        R850_SET_REG_Array(R850_Tuner_Num, 29, 0x00, 0xBE);                //SDM=1000 1110
        R850_SET_REG_Array(R850_Tuner_Num, 30, 0xE3, 0x08);            //div 8 => R30[4:2]
        R850_SET_REG_Array(R850_Tuner_Num, 30, 0xE3, 0x08);  //gm*2(24); Xtal div1 => R34[7:6] , R34[0]
        //R850_Array[R850_Tuner_Num][37] = (R850_Array[R850_Tuner_Num][37] & 0xDF) | 0x00;           //AGC ref clk (16) => same as VCO manual code
    }
    else  //no support
    {
    ...
    R850_I2C.RegAddr = 27;
    R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
    ...
    //set cap = 40p
    R850_I2C.RegAddr = 33;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x07, 0xF8);
    R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
    R850_I2C.RegAddr = 34;   //LSB
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xF7, 0x80);
    R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
    ...
    R850_I2C.RegAddr = 31;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x80, 0x40);  //manual
    R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
    ...
    //set auto
    R850_I2C.RegAddr = 31;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xBF, 0x00);
    R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
    ...
    for(i=0; i<XTAL_CHECK_SIZE; i++) //0~3
    {
        // from weak to strong
        R850_I2C.RegAddr = 34;
        R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xCF, ((UINT8)(XTAL_HIGHEST-i)<<4));  //3~0
        R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
        if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
            return RT_Fail;
        ...
        {
            *(pR850_Xtal_Pwr_tmp + R850_Tuner_Num) = i;
            break;
        }
        ...
        case R850_IMR_CAL:
                if(R850_Xtal == 24000)
                {
                    ...
                    for(InitArrayCunt = 8; InitArrayCunt<R850_REG_NUM; InitArrayCunt ++)
                    {
                        ...
                        R850_SET_REG_Array(R850_Tuner_Num, InitArrayCunt, 0x00, R850_IMR_CAL_Array[0][InitArrayCunt]);
                    }
                }
                else if(R850_Xtal == 16000)
                {
                    ...
                    for(InitArrayCunt = 8; InitArrayCunt<R850_REG_NUM; InitArrayCunt ++)
                    {
                        ...
                        R850_SET_REG_Array(R850_Tuner_Num, InitArrayCunt, 0x00, R850_IMR_CAL_Array[1][InitArrayCunt]);
                    }
                }
                else if(R850_Xtal == 27000)
                {
                    ...
                    for(InitArrayCunt = 8; InitArrayCunt<R850_REG_NUM; InitArrayCunt ++)
                    {
                        ...
                        R850_SET_REG_Array(R850_Tuner_Num, InitArrayCunt, 0x00, R850_IMR_CAL_Array[2][InitArrayCunt]);
                    }
                }
                else
                {
        ...
        case R850_LPF_CAL:
                ...
                if(R850_Xtal == 24000)
                {
                    for(InitArrayCunt = 8; InitArrayCunt<R850_REG_NUM; InitArrayCunt ++)
                    {
                        ...
                        R850_SET_REG_Array(R850_Tuner_Num, InitArrayCunt, 0x00, R850_LPF_CAL_Array[0][InitArrayCunt]);
                    }
                }
                else if(R850_Xtal == 16000)
                {
                    for(InitArrayCunt = 8; InitArrayCunt<R850_REG_NUM; InitArrayCunt ++)
                    {
                        ...
                        R850_SET_REG_Array(R850_Tuner_Num, InitArrayCunt, 0x00, R850_LPF_CAL_Array[1][InitArrayCunt]);
                    }
                }
                else if(R850_Xtal == 27000)
                {
                    for(InitArrayCunt = 8; InitArrayCunt<R850_REG_NUM; InitArrayCunt ++)
                    {
                        ...
                        R850_SET_REG_Array(R850_Tuner_Num, InitArrayCunt, 0x00, R850_LPF_CAL_Array[2][InitArrayCunt]);
                    }
                }
                else
                {
...
R850_ErrCode R850_IMR(R850_TUNER_NUM R850_Tuner_Num, UINT8 IMR_MEM, UINT8 IM_Flag, UINT8 Rev_Mode)
{
    ...
    switch(IMR_MEM)
    {
        case 0: // RingFreq = 136.0M  (16M, 24M) ;  RingFreq = 135.0M (27M) ;
            RingFreq = RingVCO/24;
            R850_SET_REG_Array(R850_Tuner_Num, 36, 0xFC, 0x02);  // ring_div1 /6 (2)	R34[6:5] => R36[1:0]
            R850_SET_REG_Array(R850_Tuner_Num, 36, 0xF3, 0x08);  // ring_div2 /4 (2)	R35[1:0] => R36[3:2]
            break;
        case 1: // RingFreq = 326M  (16M, 24M) ;  RingFreq = 324.0M (27M) ;
            RingFreq = RingVCO/10;
            R850_SET_REG_Array(R850_Tuner_Num, 36, 0xFC, 0x01);  // ring_div1 /5 (1)	R34[6:5] => R36[1:0]
            R850_SET_REG_Array(R850_Tuner_Num, 36, 0xF3, 0x04);  // ring_div2 /2 (1)	R35[1:0] => R36[3:2]
            break;
        case 2: // RingFreq = 544M  (16M, 24M) ;  RingFreq = 540.0M (27M) ;
            RingFreq = RingVCO/6;
            R850_SET_REG_Array(R850_Tuner_Num, 36, 0xFC, 0x02);  // ring_div1 /6 (2)	R34[6:5] => R36[1:0]
            R850_SET_REG_Array(R850_Tuner_Num, 36, 0xF3, 0x00);  // ring_div2 /1 (0)	R35[1:0] => R36[3:2]
            break;
        case 3: // RingFreq = 816M  (16M, 24M) ;  RingFreq = 810.0M (27M) ;
            RingFreq = RingVCO/4;
            R850_SET_REG_Array(R850_Tuner_Num, 36, 0xFC, 0x00);  // ring_div1 /4 (0)	R34[6:5] => R36[1:0]
            R850_SET_REG_Array(R850_Tuner_Num, 36, 0xF3, 0x00);  // ring_div2 /1 (0)	R35[1:0] => R36[3:2]
            break;
        case 4: // RingFreq = 136M  (16M, 24M) ;  RingFreq = 135.0M (27M) ;
            RingFreq = RingVCO/16;
            R850_SET_REG_Array(R850_Tuner_Num, 36, 0xFC, 0x00); // ring_div1 /4 (0)	R34[6:5] => R36[1:0]
            R850_SET_REG_Array(R850_Tuner_Num, 36, 0xF3, 0x08);  // ring_div2 /4 (2)	R35[1:0] => R36[3:2]
            break;
    ...
    R850_SET_REG_Array(R850_Tuner_Num, 35, 0xE0, divnum_ring);   //ring_div_num, R34[4:0]

    if(RingVCO>=3200000)
        R850_SET_REG_Array(R850_Tuner_Num, 35, 0xBF, 0x00);   //vco_band=high, R34[7]=0 => R35[6]
     else
        R850_SET_REG_Array(R850_Tuner_Num, 35, 0xFF, 0x40);      //vco_band=low, R34[7]=1 => R35[6]
...
*/
    if(Rev_Mode==R850_IMR_NOR)
    {
        ...
        if(R850_PLL(R850_Tuner_Num, (RingFreq - R850_IMR_IF), R850_STD_SIZE) != RT_Success)  //IMR PLL
         ...
         //Img_R = normal
        R850_I2C.RegAddr = 19;
        R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xEF, 0x00);  //R20[7]=0 => R19[4]=0
        R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
        if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
            return RT_Fail;
        // Mixer Amp LPF=7 (0 is widest)
        R850_I2C.RegAddr = 19;
        R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xF8, R850_Mixer_Amp_LPF_IMR_CAL);
        R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
        if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
            return RT_Fail;
        ...
        if(IMR_MEM==4)
        {
            //output atten
             R850_I2C.RegAddr = 36;
             R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xCF, 0x10);  // R41[3:0]=0
             R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
             if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
                 return RT_Fail;
        }
        else
        {
             //output atten
             R850_I2C.RegAddr = 36;
             R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xCF, 0x30);
             R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
             if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
                 return RT_Fail;
        }
        ...
        //Mixer Gain = 8
         R850_I2C.RegAddr = 41;
         R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xF0, 0x08);  //R41[3:0]=0
         R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
         if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
             return RT_Fail;
    ...
    else
    {
        ...
        //Img_R = inverse
        R850_I2C.RegAddr = 19;
        R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFF, 0x10);  //R20[7]=0 => R19[4]=1
        R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
        if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
             return RT_Fail;
        ...
        if(IMR_MEM==9)
        {
            //Mixer Gain = 7
             R850_I2C.RegAddr = 41;
             R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xF0, 7);  // R41[3:0]=0
             R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
             if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
                 return RT_Fail;
        ...
        else
        {
            //Mixer Gain = 6
             R850_I2C.RegAddr = 41;
             R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xF0, 6);  // R41[3:0]=0
             R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
             if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
                 return RT_Fail;
             ...
             //output atten
             R850_I2C.RegAddr = 36;
             R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xCF, 0x30);  // R41[3:0]=0
             R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
             if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
                 return RT_Fail;
        }
    ...
    {
        *(pR850_IMR_Cal_Result + R850_Tuner_Num) = 1; //fail
    }
    return RT_Success;
}
...
R850_ErrCode R850_PLL(R850_TUNER_NUM R850_Tuner_Num, UINT32 LO_Freq, R850_Standard_Type R850_Standard)//ok
{
    ...
    // VCO current = 0 (max)
    R850_I2C.RegAddr = 32;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFC, 0x00);

    // VCO power = auto
    R850_I2C.RegAddr = 46;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xBF, 0x40);

    // HfDiv Buf = 6dB buffer
    R850_I2C.RegAddr = 37;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xEF, 0x00);
    ...
    // PLL LDOA=2.2V
    R850_I2C.RegAddr = 11;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xF3, 0x00);

    // PFD DLDO=4mA
    R850_I2C.RegAddr = 12;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x3F, 0x00);

    // DLDO2=3mA
    R850_I2C.RegAddr = 11;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xCF, 0x10);

    // HF Fiv LDO=7mA (new bonding set this off)
    R850_I2C.RegAddr = 9;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xF9, 0x00);

      //------ Xtal freq depend setting: Xtal Gm & AGC ref clk --------//
    if(R850_Xtal==24000)
    {
        R850_SET_REG_Array(R850_Tuner_Num, 34, 0x3F, 0x00);
        R850_SET_REG_Array(R850_Tuner_Num, 37, 0xDF, 0x20);
    }
    else if(R850_Xtal==16000)
    {
        R850_SET_REG_Array(R850_Tuner_Num, 34, 0x3F, 0x80);
        R850_SET_REG_Array(R850_Tuner_Num, 37, 0xDF, 0x00);
    }
    else if(R850_Xtal==27000)
    {
        R850_SET_REG_Array(R850_Tuner_Num, 34, 0x3F, 0x00);
...
/*#elif(R850_SHARE_XTAL_SLAVE==TRUE)
        ...
        //gm off
        R850_Array[R850_Tuner_Num][34] = (R850_Array[R850_Tuner_Num][34] | 0xC0);  //gm off  R34[7:6]:11
*/
#else
        if(LO_Freq<100000) //Xtal PW : Low
        {
            if(*(pR850_Xtal_Pwr + R850_Tuner_Num) <= XTAL_LOW )
            {
                XTAL_POW = 2;
            }
            else
            {
                XTAL_POW = 3 - *(pR850_Xtal_Pwr + R850_Tuner_Num);
            }
        }
        else if((110000<=LO_Freq) && (LO_Freq<130000)) //Xtal PW: High
        {
            if(*(pR850_Xtal_Pwr + R850_Tuner_Num) <= XTAL_HIGH )
            {
                XTAL_POW = 1;
            }
            else
    ...
    R850_I2C.RegAddr = 34;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xCF, (XTAL_POW<<4));    //R32[2:1] => R34[5:4]
    ...
    R850_I2C.RegAddr = 30;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x1F, CP_CUR);    //    => R30[7:5]

    R850_I2C.RegAddr = 37;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFD, CP_OFFSET);    //    => R37[1]
    ...
    {
        PLL_Ref = R850_Xtal;
        R850_SET_REG_Array(R850_Tuner_Num, 34, 0xFC, 0x00); //b7:2nd_div2=0, b6:1st_div2=0     R32[7:6] => R34[1:0]
    }
    else if(R850_XtalDiv==XTAL_DIV1_2)
    {
        PLL_Ref = R850_Xtal/2;
        R850_SET_REG_Array(R850_Tuner_Num, 34, 0xFC, 0x02); //1st_div2=0(R34[0]), 2nd_div2=1(R34[1])
    }
    else if(R850_XtalDiv==XTAL_DIV2_1)
    {
        PLL_Ref = R850_Xtal/2;
        R850_SET_REG_Array(R850_Tuner_Num, 34, 0xFC, 0x01); //1st_div2=1(R34[0]), 2nd_div2=0(R34[1])
    }
    else if(R850_XtalDiv==XTAL_DIV4)  //24MHz
    {
        PLL_Ref = R850_Xtal/4;
        R850_SET_REG_Array(R850_Tuner_Num, 34, 0xFC, 0x03); //b7:2nd_div2=1, b6:1st_div2=1  R32[7:6] => R34[1:0]
    }


    //IQ gen current R10[7] => R11[0]
    R850_I2C.RegAddr = 11;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFE, IQGen_Cur);

    //Out Buf Bias R10[6:5] => R45[3:2]
    R850_I2C.RegAddr = 45;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xF3, (OutBuf_Bias<<2));

    //BiasI R29[2] => R46[0]
    R850_I2C.RegAddr = 46;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFE, IQBias);

    //IQLoad R36[5] => R46[1]
    R850_I2C.RegAddr = 46;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFD, (IQLoad<<1));

    //BiasHF R29[7:6] => R32[1:0]
    R850_I2C.RegAddr = 32;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFC, BiasHf);

    //SDM_RES R30[7] => R32[4]
    R850_I2C.RegAddr = 32;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xEF, (SDM_RES<<4));

    //NS_RES  R36[0]  => R17[7]
    R850_I2C.RegAddr = 17;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x7F, (NS_RES<<7));

    //Divider num R35[5:3] => R30[4:2]
    R850_I2C.RegAddr = 30;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xE3, (DivNum << 2));
    ...
    R850_I2C.RegAddr = 27;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x80, Ni);      //R26[6:0] => R27[6:0]

    R850_I2C.RegAddr = 30;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFC, Si);  //R35[7:6] => R30[1:0]

    //pw_sdm & pw_dither
    R850_I2C.RegAddr = 32;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x3F, 0x00);        //R29[1:0] => R32[7:6]
    if(VCO_Fra == 0)
    {
         R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFF, 0xC0);
    }
    ...
    R850_I2C.RegAddr = 29;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x00, (UINT8)SDM16to9);  //R28 => R29

    R850_I2C.RegAddr = 28;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x00, (UINT8)SDM8to1);    //R27 => R28
    ...
    {
        R850_I2C_Len.Data[PllArrayCunt] = R850_GET_REG_Array(R850_Tuner_Num, PllArrayCunt + 8);
    }
    if(I2C_Write_Len(R850_Tuner_Num, &R850_I2C_Len) != RT_Success)
        return RT_Fail;
    ...
    R850_I2C.RegAddr = 47;
    if(R850_Chip == R850_MP)
        R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFC, 0x02);
    else
        R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFD, 0x02);
    R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
...
R850_ErrCode R850_MUX(R850_TUNER_NUM R850_Tuner_Num, UINT32 LO_KHz, UINT32 RF_KHz, R850_Standard_Type R850_Standard)
{
    ...
    *(pFreq_Info1 + R850_Tuner_Num) = R850_Freq_Sel(LO_KHz, RF_KHz, R850_Standard);
    pFreq_Info = pFreq_Info1 + R850_Tuner_Num;

    // TF_DIPLEXER
    R850_I2C.RegAddr = 14;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xF3, (pFreq_Info->TF_DIPLEXER) << 2);  //LPF    => R14[3:2]
    R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
    ...
    R850_I2C.RegAddr = 16;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xE0, (pFreq_Info->TF_HPF_BPF) | ((pFreq_Info->TF_HPF_CNR) << 3));  // R16[2:0], R16[4:3]
    R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
    ...
    // RF Polyfilter
    R850_I2C.RegAddr = 18;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFC, pFreq_Info->RF_POLY);  //R17[6:5] => R18[1:0]
    R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
    ...
    // LNA Cap
    R850_I2C.RegAddr = 14;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x0F, pFreq_Info->LPF_CAP << 4);      //R16[3:0]    =>    R14[7:4]
    R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
    ...
    // LNA Notch
    R850_I2C.RegAddr = 15;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xF0, pFreq_Info->LPF_NOTCH);  //R16[7:4]  =>  R15[3:0]
    R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
    ...
    //Set_IMR
    if( (*(pR850_IMR_done_flag + R850_Tuner_Num) == TRUE) && (*(pR850_IMR_Cal_Result + R850_Tuner_Num)==0))
    {
        if(R850_Mixer_Mode==R850_IMR_NOR)
        {
            Reg_IMR_Gain = R850_IMR_Data[R850_Tuner_Num][pFreq_Info->IMR_MEM_NOR].Gain_X & 0x2F;   //R20[4:0] => R20[3:0]
            Reg_IMR_Phase = R850_IMR_Data[R850_Tuner_Num][pFreq_Info->IMR_MEM_NOR].Phase_Y & 0x2F; //R21[4:0] => R21[3:0]
            Reg_IMR_Iqcap = R850_IMR_Data[R850_Tuner_Num][pFreq_Info->IMR_MEM_NOR].Iqcap;                  //0,1,2
        }
        else
        {
            Reg_IMR_Gain = R850_IMR_Data[R850_Tuner_Num][pFreq_Info->IMR_MEM_REV].Gain_X & 0x2F;   //R20[4:0] => R20[3:0]
            Reg_IMR_Phase = R850_IMR_Data[R850_Tuner_Num][pFreq_Info->IMR_MEM_REV].Phase_Y & 0x2F; //R21[4:0] => R21[3:0]
            Reg_IMR_Iqcap = R850_IMR_Data[R850_Tuner_Num][pFreq_Info->IMR_MEM_REV].Iqcap;                  //0,1,2
        }
    }
    else
    {
    ...
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xD0, (Reg_IMR_Gain & 0x2F));
    R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
    ...
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xD0, (Reg_IMR_Phase & 0x2F));
    R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
    ...
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x3F, (Reg_IMR_Iqcap<<6));
    R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
...
R850_ErrCode R850_IQ(R850_TUNER_NUM R850_Tuner_Num, R850_SectType* IQ_Pont)
{
    ...
    R850_I2C.RegAddr = 41;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFF, 0xF0);
    R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
    ...
    Compare_IQ[0].Gain_X  = R850_GET_REG_Array(R850_Tuner_Num, R850_IMR_GAIN_REG) & 0xD0;
    Compare_IQ[0].Phase_Y = R850_GET_REG_Array(R850_Tuner_Num, R850_IMR_PHASE_REG) & 0xD0;
    ...
    *IQ_Pont = Compare_IQ[0];
    ...
    R850_I2C.RegAddr = R850_IMR_GAIN_REG;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xD0, 0x00);
    R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
...
R850_ErrCode R850_IMR_Cross(R850_TUNER_NUM R850_Tuner_Num, R850_SectType* IQ_Pont, UINT8* X_Direct)
{
    ...
    R850_SectType Compare_Temp;
    UINT8 CrossCount = 0;
    UINT8 RegGain = R850_GET_REG_Array(R850_Tuner_Num, R850_IMR_GAIN_REG) & 0xD0;
    UINT8 RegPhase = R850_GET_REG_Array(R850_Tuner_Num, R850_IMR_PHASE_REG) & 0xD0;
...
R850_ErrCode R850_IMR_Iqcap(R850_TUNER_NUM R850_Tuner_Num, R850_SectType* IQ_Point)
{
    ...
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x00, R850_I2C.Data);
    if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
    ...
    for(i=0; i<3; i++)
    {
        ...
        R850_I2C.RegAddr = R850_IMR_IQCAP_REG;
        R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x3F, (Compare_Temp.Iqcap<<6));
        R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
        if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
               return RT_Fail;
...
R850_ErrCode R850_SetStandard(R850_TUNER_NUM R850_Tuner_Num, R850_Standard_Type RT_Standard)
{
    ...
    UINT8* pFil_Cal_flag = NULL;
    UINT8* pFil_Cal_code = NULL;
    UINT8* pFil_Cal_BW = NULL;
    UINT8* pFil_Cal_LpfLsb = NULL;

    ppre_standard = pR850_pre_standard + R850_Tuner_Num;
    pSys_Info = pSys_Info1 + R850_Tuner_Num;
    pFil_Cal_flag = pR850_Fil_Cal_flag + R850_Tuner_Num * R850_STD_SIZE;
    pFil_Cal_code = pR850_Fil_Cal_code + R850_Tuner_Num * R850_STD_SIZE;
    pFil_Cal_BW = pR850_Fil_Cal_BW + R850_Tuner_Num * R850_STD_SIZE;
    pFil_Cal_LpfLsb = pR850_Fil_Cal_LpfLsb + R850_Tuner_Num * R850_STD_SIZE;
    ...
    {
        *(pFil_Cal_code + RT_Standard) = R850_Filt_Cal_ADC(R850_Tuner_Num, pSys_Info->FILT_CAL_IF, pSys_Info->BW, u1FilCalGap);
        *(pFil_Cal_BW + RT_Standard) = R850_Bandwidth;
        *(pFil_Cal_flag + RT_Standard) = TRUE;
        *(pFil_Cal_LpfLsb + RT_Standard) = R850_Lpf_Lsb;

        //protection
        if(*(pR850_IMR_Cal_Result + R850_Tuner_Num)==1) //fail
        {
    ...
    // Set LPF Lsb bit
    R850_I2C.RegAddr = 23;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFE, *(pFil_Cal_LpfLsb + RT_Standard));
    // Set LPF fine code
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xE1, *(pFil_Cal_code + RT_Standard) << 1);
    // Set LPF coarse BW
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x9F, *(pFil_Cal_BW + RT_Standard) << 5);
    //Set HPF notch  R23[7]
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x7F, pSys_Info->HPF_NOTCH << 7);
    R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
    *(pR850_Atsc_LPF + RT_Standard) = (*(pFil_Cal_code + RT_Standard)<<1) + *(pFil_Cal_LpfLsb + RT_Standard);


    // Set HPF corner
    R850_I2C.RegAddr = 24;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x0F, pSys_Info->HPF_COR << 4);    //R24[3:0] => R24[7:4]
    R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
    *(pR850_Atsc_HPF + RT_Standard) = pSys_Info->HPF_COR;


    // Set Filter Auto Ext
    R850_I2C.RegAddr = 18;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xBF, pSys_Info->FILT_EXT_ENA << 6);  // =>R18[6]
    R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
    ...
    R850_I2C.RegAddr = 24;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xF3, pSys_Info->FILT_COMP << 2);
    R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
    ...
    // Set AGC clk R47[3:2]
    R850_I2C.RegAddr = 47;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xF3, pSys_Info->AGC_CLK << 2);
    R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
    ...
    {
        R850_I2C.RegAddr = 46;
        R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xEF, pSys_Info->IMG_GAIN << 4);
        R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
        if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
            return RT_Fail;
    }
    ...
    {
        R850_I2C.RegAddr = 44;
        R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFE, (pSys_Info->IMG_GAIN & 0x02) >> 1);
        R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
        if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
            return RT_Fail;
        R850_I2C.RegAddr = 46;
        R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xEF, (pSys_Info->IMG_GAIN & 0x01) << 4);
        R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
        if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
            return RT_Fail;
    }
...
UINT8  R850_Filt_Cal_ADC(R850_TUNER_NUM R850_Tuner_Num, UINT32 IF_Freq, UINT8 R850_BW, UINT8 FilCal_Gap)
{
     ...
     {
        R850_I2C.RegAddr = 41;
        R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x0F, (LPF_Count<<4));
        R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
        if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
            return RT_Fail;
     ...
     for(i=initial_cnt; i<3; i++)
     {
         ...
         R850_I2C.RegAddr = 23;
         R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xCF, (R850_Bandwidth<<5));
         R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
         if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
            return RT_Fail;
         ...
         R850_I2C.RegAddr = 23;
         R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xE1, 0);
         R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
         if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
              return RT_Fail;
         ...
         R850_I2C.RegAddr = 23;
         R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xE1, (13<<1));  //code 26
         R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
         if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
              return RT_Fail;
     ...
     {
         R850_I2C.RegAddr = 23;
         R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xE1, (u1FilterCode<<1));
         R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
         if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
              return RT_Fail;
     ...
     {
         R850_I2C.RegAddr = 23;
         R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xE0, ((u1FilterCodeResult-1)<<1) | 0x01);
         R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
         if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
              return RT_Fail;
...
R850_ErrCode R850_SetFrequency(R850_TUNER_NUM R850_Tuner_Num, R850_Set_Info R850_INFO) //Write Multi byte
{
    ...
    pSysFreq_Info = pSysFreq_Info1 + R850_Tuner_Num;
    pFreq_Info = pFreq_Info1 + R850_Tuner_Num;
    pSys_Info = pSys_Info1 + R850_Tuner_Num;
     ...
     R850_I2C.RegAddr = 19;
     R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xEF, (Img_R<<4));
     R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
     if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
    ...
    if(R850_External_LNA == 0)    //External LNA Disable => depend on setting
        R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xBF, ((pSysFreq_Info->NA_PWR_DET)<<6));
    ...
    if(R850_External_LNA == 0)    //External LNA Disable => depend on initial value
        R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xDF, (R850_iniArray[0][R850_I2C.RegAddr] & 0x20));
    else    //External LNA Enable => fixed
        R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFF, 0x20);


    //LNA_NRB_DET R11[7]
    R850_I2C.RegAddr = 11;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x7F, ((pSysFreq_Info->LNA_NRB_DET)<<7));

    //LNA
    // LNA_TOP R38[2:0]
    R850_I2C.RegAddr = 38;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xF8, (7 - pSysFreq_Info->LNA_TOP));

    // LNA VTL/H R39[7:0]
    R850_I2C.RegAddr = 39;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x00, pSysFreq_Info->LNA_VTL_H);

    // RF_LTE_PSG R17[4]
    R850_I2C.RegAddr = 17;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xEF, (pSysFreq_Info->RF_LTE_PSG << 4));

    //RF
    // RF TOP R38[6:4]
    R850_I2C.RegAddr = 38;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x8F, (7 - pSysFreq_Info->RF_TOP) << 4);

    // RF VTL/H R42[7:0]
    R850_I2C.RegAddr = 42;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x00, pSysFreq_Info->RF_VTL_H);

    // RF Gain Limt  (MSB:R18[2] & LSB:R16[6])
    switch(pSysFreq_Info->RF_GAIN_LIMIT)
    {
    case 0://'b00
        R850_SET_REG_Array(R850_Tuner_Num, 18, 0xFB, 0x00);
        R850_SET_REG_Array(R850_Tuner_Num, 16, 0xBF, 0x00);
        break;
    case 1://'b01
        R850_SET_REG_Array(R850_Tuner_Num, 18, 0xFB, 0x00);
        R850_SET_REG_Array(R850_Tuner_Num, 16, 0xFF, 0x40);
        break;
    case 2://'b10
        R850_SET_REG_Array(R850_Tuner_Num, 18, 0xFF, 0x02);
        R850_SET_REG_Array(R850_Tuner_Num, 16, 0xBF, 0x00);
        break;
    case 3://'b11
        R850_SET_REG_Array(R850_Tuner_Num, 18, 0xFF, 0x02);
    ...
    R850_I2C.RegAddr = 19;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xF8, pSysFreq_Info->MIXER_AMP_LPF);

    // MIXER TOP R40[3:0]
    R850_I2C.RegAddr = 40;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xF0, (15 - pSysFreq_Info->MIXER_TOP));

    // Filter TOP R44[3:0] for MP, R44[3:1] for MT1
    R850_I2C.RegAddr = 44;
    if(R850_Chip == R850_MP)
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xF0, (15 - pSysFreq_Info->FILTER_TOP));
    else
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xF1, ((7 - pSysFreq_Info->FILTER_TOP)<<1));

    //FILT_3TH_LPF_CUR=0;	//R10[4] [high (0), low (1)]
    R850_I2C.RegAddr = 10;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xEF, ((pSysFreq_Info->FILT_3TH_LPF_CUR)<<4));

    // 3th_LPF_Gain R24[1:0]
    R850_I2C.RegAddr = 24;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFC, pSysFreq_Info->FILT_3TH_LPF_GAIN);

    // MIXER VTH & Filter VTH  R41[7:0]
    R850_I2C.RegAddr = 41;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x00, pSysFreq_Info->MIXER_VTH | pSysFreq_Info->FILTER_VTH);

    // MIXER VTL & Filter VTL R43[7:0]
    R850_I2C.RegAddr = 43;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x00, pSysFreq_Info->MIXER_VTL | pSysFreq_Info->FILTER_VTL);

    // Mixer Gain Limt R22[7:6]
    R850_I2C.RegAddr = 22;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x3F, ((pSysFreq_Info->MIXER_GAIN_LIMIT) << 6));

    // MIXER_DETBW_LPF R46[7]
    R850_I2C.RegAddr = 46;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x7F, ((pSysFreq_Info->MIXER_DETBW_LPF) << 7));

    //Discharge
    //LNA_RF Discharge Mode  (R45[1:0]=2'b01; R9[6]=1)	=> (R45[1:0]=0'b00; R31[0]=1 ;R32[5]=1)
    if(pSysFreq_Info->LNA_RF_DIS_MODE==0)  //auto (normal)
    {
        R850_SET_REG_Array(R850_Tuner_Num, 45, 0xFC, 0x00);	//00
        R850_SET_REG_Array(R850_Tuner_Num, 31, 0xFE, 0x01);   //1
        R850_SET_REG_Array(R850_Tuner_Num, 32, 0xDF, 0x20);	//1
    }
    else if(pSysFreq_Info->LNA_RF_DIS_MODE==1)  //both(fast+slow)
    {
        R850_SET_REG_Array(R850_Tuner_Num, 45, 0xFC, 0x03);	//11
        R850_SET_REG_Array(R850_Tuner_Num, 31, 0xFE, 0x01);   //1
        R850_SET_REG_Array(R850_Tuner_Num, 32, 0xDF, 0x20);	//1
    }
    ...
    {
        R850_SET_REG_Array(R850_Tuner_Num, 45, 0xFC, 0x03); //11
        R850_SET_REG_Array(R850_Tuner_Num, 31, 0xFE, 0x00);   //0
        R850_SET_REG_Array(R850_Tuner_Num, 32, 0xDF, 0x00); //0
    }
    ...
    R850_I2C.RegAddr = 31;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFD, (pSysFreq_Info->LNA_RF_CHARGE_CUR) << 1);

    //LNA_RF_dis current  R13[5]
    R850_I2C.RegAddr = 13;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xDF, (pSysFreq_Info->LNA_RF_DIS_CURR) << 5);

    //RF_slow disch(5:4) / fast disch(7:6)  //(R45[7:4])
    R850_I2C.RegAddr = 45;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x0F, (pSysFreq_Info->RF_DIS_SLOW_FAST) << 4);

    //LNA slow disch(5:4) / fast disch(7:6) =>R44[7:4]
    R850_I2C.RegAddr = 44;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x0F, (pSysFreq_Info->LNA_DIS_SLOW_FAST) << 4);

    //BB disch current R25[6]
    R850_I2C.RegAddr = 25;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xBF, (pSysFreq_Info->BB_DIS_CURR) << 6);

    //Mixer/Filter disch  R37[7:6]
    R850_I2C.RegAddr = 37;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x3F, (pSysFreq_Info->MIXER_FILTER_DIS) << 6);

    //BB Det Mode  R37[2]
    R850_I2C.RegAddr = 37;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFB, (pSysFreq_Info->BB_DET_MODE) << 2);

    //Polyphase
    //ENB_POLY_GAIN  R25[1]
    R850_I2C.RegAddr = 25;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFD, (pSysFreq_Info->ENB_POLY_GAIN) << 1);

    //NRB
    // NRB TOP R40[7:4]
    R850_I2C.RegAddr = 40;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x0F, (15 - pSysFreq_Info->NRB_TOP) << 4);

    //NRB LPF & HPF BW  R26[7:6 & 3:2]
    R850_I2C.RegAddr = 26;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x33, (pSysFreq_Info->NRB_BW_HPF) << 2 | (pSysFreq_Info->NRB_BW_LPF) << 6);

    //NBR Image TOP adder R46[3:2]
    R850_I2C.RegAddr = 46;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xF3, (pSysFreq_Info->IMG_NRB_ADDER) << 2);

    //VGA
    //HPF_COMP R13[2:1]
    R850_I2C.RegAddr = 13;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xF9, (pSysFreq_Info->HPF_COMP) << 1);

    //FB_RES_1ST  //R21[4]
    R850_I2C.RegAddr = 21;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xEF, (pSysFreq_Info->FB_RES_1ST) << 4);

    //DEGLITCH_CLK;	R38[3]
    R850_I2C.RegAddr = 38;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xF7, (pSysFreq_Info->DEGLITCH_CLK) << 3);

    //NAT_HYS;R35[6]
    R850_I2C.RegAddr = 35;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xBF, (pSysFreq_Info->NAT_HYS) << 6);

    //NAT_CAIN;R31[4:5]
    R850_I2C.RegAddr = 31;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xCF, (pSysFreq_Info->NAT_CAIN) << 4);

    //PULSE_HYS;R26[1:0]
    R850_I2C.RegAddr = 26;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFC, (pSysFreq_Info->PULSE_HYS));

    //FAST_DEGLITCH;R36[3:2]
    R850_I2C.RegAddr = 36;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xF3, (pSysFreq_Info->FAST_DEGLITCH) << 2);

    //PUL_RANGE_SEL;R31[2]
    R850_I2C.RegAddr = 31;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFB, (pSysFreq_Info->PUL_RANGE_SEL) << 2);

    //PUL_FLAG_RANGE;R35[1:0]
    R850_I2C.RegAddr = 35;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFC, (pSysFreq_Info->PUL_FLAG_RANGE));

    //PULG_CNT_THRE;R17[1]
    R850_I2C.RegAddr = 17;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFD, (pSysFreq_Info->PULG_CNT_THRE) << 1);

    //FORCE_PULSE;R46[5]
    R850_I2C.RegAddr = 46;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xDF, (pSysFreq_Info->FORCE_PULSE) << 5);

    //FLG_CNT_CLK;R47[7:6]
    R850_I2C.RegAddr = 47;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xBF, (pSysFreq_Info->FLG_CNT_CLK) << 6);

    //NATG_OFFSET;R36[5:4]
    R850_I2C.RegAddr = 36;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xCF, (pSysFreq_Info->NATG_OFFSET) << 4);
    ...
    {
        R850_I2C.RegAddr = 36;
        R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x3F, 0x40);
    }

    //IF AGC1
    R850_I2C.RegAddr = 25;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xDF, 0x00);
...
*/
    ...
    {
        R850_SET_REG_Array(R850_Tuner_Num, 34, 0xFF, 0x04); //no clk out  R34[2] = 1
    }
    else
    {
        R850_SET_REG_Array(R850_Tuner_Num, 34, 0xFB, 0x00); ////clk out  R34[2] = 0
    }
    ...
    {
        R850_I2C_Len.Data[SetFreqArrayCunt] = R850_GET_REG_Array(R850_Tuner_Num, SetFreqArrayCunt + 8);
    }
    if(I2C_Write_Len(R850_Tuner_Num, &R850_I2C_Len) != RT_Success)
        return RT_Fail;


    *(pR850_Atsc_Mode + R850_Tuner_Num) = 0;
    *(pR850_Atsc_Mode_Pre + R850_Tuner_Num) = 0;
...
R850_ErrCode R850_SetPllData_Mode(R850_TUNER_NUM R850_Tuner_Num, R850_Set_Info R850_INFO, R850_TuningMode R850_Tune_Mode)
{
    UINT8 agc_cont = 0;
    UINT8 LNA_Gain;
    ...
    if(R850_Tune_Mode == R850_CHANNEL_CHANGE)
    {
        ...
        do
        {
            ...
            R850_Delay_MS(R850_Tuner_Num, 10);
            if(I2C_Read_Len(R850_Tuner_Num, &R850_I2C_Len) != RT_Success)
            {
                I2C_Read_Len(R850_Tuner_Num, &R850_I2C_Len);
            }
        ...
        //Set AGC slow
        if(R850_AGC_Slow(R850_Tuner_Num, R850_INFO.R850_Standard) != RT_Success)
            return RT_Fail;
    }
      return RT_Success;
}
...
R850_ErrCode R850_Standby(R850_TUNER_NUM R850_Tuner_Num, R850_LoopThrough_Type R850_STANDBY_LT)
{
    ...
    {
        R850_SBY[R850_Tuner_Num][i]=R850_GET_REG_Array(R850_Tuner_Num, i);
    }

     //All, LDO , LNA
     R850_I2C.RegAddr = 8;
     R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xC3, 0x03);
     R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
     if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
            return RT_Fail;
     ...
     R850_I2C.RegAddr = 9;
     R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x00, 0xEE);
     R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
     if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
            return RT_Fail;
     ...
     R850_I2C.RegAddr = 10;
     R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xBB, 0xB9);
     R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
     if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
            return RT_Fail;
     ...
     R850_I2C.RegAddr = 11;
     R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x00, 0xFE);
     R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
     if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
            return RT_Fail;
     ...
     R850_I2C.RegAddr = 12;
     R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xF0, 0x0F);
     R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
     if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
            return RT_Fail;
     ...
     if(R850_STANDBY_LT==LT_ON)
     {
        ...
        R850_SET_REG_Array(R850_Tuner_Num, 8, 0x3F, 0x00);
        //Pulse LT off R10[1]=0
        R850_SET_REG_Array(R850_Tuner_Num, 10, 0xFD, 0x00);
     }
     else
     {
        //LT Sel: active LT(after LNA) R8[6]=1 ; acLT PW: OFF R8[7]=0
        R850_SET_REG_Array(R850_Tuner_Num, 8, 0x7F, 0x40);
        //Pulse LT off R10[1]=0
        R850_SET_REG_Array(R850_Tuner_Num, 10, 0xFD, 0x00);
     }
    ...
    //LNA manual mode and gain =31; R13[0]=1 , R13[5]=1(Gain MSB)
     R850_I2C.RegAddr = 13;
     R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFF, 0x21);
     R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
     if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
            return RT_Fail;
    ...
    //LNA manual Gain = 31  R39[7:4]
     ...
     R850_I2C.RegAddr = 14;
     R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xF3, 0x04);
     R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
     if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
            return RT_Fail;
     ...
     //VGA code mode  R25[2]
     R850_I2C.RegAddr = 25;
     R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFF, 0x04);
     R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
     if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
            return RT_Fail;
     ...
     R850_I2C.RegAddr = 17;
     R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFF, 0x40);
     R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
     if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
            return RT_Fail;

     R850_I2C.RegAddr = 42;
     R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x0F, 0x00);
     R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
     if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
            return RT_Fail;
     ...
     R850_I2C.RegAddr = 8;
     R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFF, 0x30);
     R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
     if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
            return RT_Fail;
...
R850_ErrCode R850_WakeUp(R850_TUNER_NUM R850_Tuner_Num)
{
    ...
    for(i = 0; i<R850_REG_NUM; i ++)
    {
        ...
        R850_SET_REG_Array(R850_Tuner_Num, i, 0x00, R850_SBY[R850_Tuner_Num][i]);
    }
    if(I2C_Write_Len(R850_Tuner_Num, &R850_I2C_Len) != RT_Success)
        return RT_Fail;
    ...
    R850_I2C.RegAddr = 10;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x7F, 0x00);
    R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
    ...
    R850_I2C.RegAddr = 12;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x3F, PDF_dldo);
    R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
    ...
    R850_I2C.RegAddr = 11;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xC3, (PLL_LDO_A|PLL_LDO_D) );  //R9[5]=0
    R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
...
R850_ErrCode R850_SetLoop_Through(R850_TUNER_NUM R850_Tuner_Num, UINT8 u8Cur_tuner_type , R850_LoopThrough_Type R850_LT)
{
    UINT8 u8Reg8_ALL_PW, u8Reg12_AGC_PW, u8Reg11_PLL_LDO;


    u8Reg8_ALL_PW = R850_GET_REG_Array(R850_Tuner_Num, 8) & 0x01;   //All PW
    u8Reg12_AGC_PW = R850_GET_REG_Array(R850_Tuner_Num, 12) & 0x01;   //AGC PW
    u8Reg11_PLL_LDO = (R850_GET_REG_Array(R850_Tuner_Num, 11) & 0x3C);   //PLL LDO
    ...
    else  // Atcive mode
    {
        ...
        {
            //LT Sel: active LT(after LNA) R8[6]=1 ; acLT PW: ON R8[7]=1
            R850_SET_REG_Array(R850_Tuner_Num, 8, 0xFF, 0xC0);
            //Pulse LT on R10[1]=1
            R850_SET_REG_Array(R850_Tuner_Num, 10, 0xFF, 0x02);
        }
        else
        {
            //LT Sel: active LT(after LNA) R8[6]=1 ; acLT PW: OFF R8[7]=0
...
R850_ErrCode R850_RfGainMode(R850_TUNER_NUM R850_Tuner_Num, R850_RF_Gain_TYPE R850_RfGainType)
{
    ...
    if(R850_RfGainType==RF_MANUAL)
    {
        //LNA auto off R13[0]
         R850_I2C.RegAddr = 13;
        R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFF, 0x01);
        R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
         if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
               return RT_Fail;
         ...
         R850_I2C.RegAddr = 12;
        R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFF, 0x80);
        R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
         if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
               return RT_Fail;
        ...
        R850_I2C.RegAddr = 13;
        R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xDF, (LnaGain & 0x10));
        R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
        if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
           return RT_Fail;
        R850_I2C.RegAddr = 39;
        R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x0F, ((LnaGain & 0x0F)<<4));
        R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
        if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
           return RT_Fail;
        ...
        R850_I2C.RegAddr = 0x1A;
        R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x0F, RfGain<<4);
        R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
        if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
           return RT_Fail;
        ...
        R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xF0, MixerGain);
        R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
        if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
           return RT_Fail;
    }
    else
    {
        //LNA auto on R13[0]
         R850_I2C.RegAddr = 13;
         R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFE, 0x00);
         R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
         if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
               return RT_Fail;
...
R850_ErrCode R850_AGC_Slow(R850_TUNER_NUM R850_Tuner_Num, R850_Standard_Type R850_Standard)
{
    UINT8   Mixer_vth;
     ...
     R850_I2C.RegAddr = 47;
     R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xF3, 0x0C);
     R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
     if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
           return RT_Fail;
    ...
    {
        //Mixer VTH R41[3:0]
        Mixer_vth = (R850_GET_REG_Array(R850_Tuner_Num, 41)) & 0x0F;
        if(Mixer_vth < 10)
        {
            //Mixer VTH = 1.34V R41[3:0] = 1010
            R850_I2C.RegAddr = 41;
            R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xF0, 0x0A);
            R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
            if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
                return RT_Fail;
        }
...
R850_ErrCode R850_SetLpfBw(R850_TUNER_NUM R850_Tuner_Num, UINT8 LPF_BW)
{
    ...
    R850_I2C.RegAddr = 23;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x9F, u1LpfBw);
    R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
...
R850_ErrCode R850_SetLpfOffset(R850_TUNER_NUM R850_Tuner_Num, UINT8 LPF_Offset)
{
    // Set LPF fine code
    R850_I2C.RegAddr = 23;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xE0, LPF_Offset);
    R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
...
R850_ErrCode R850_SetHpfOffset(R850_TUNER_NUM R850_Tuner_Num, UINT8 HPF_Offset)
{
    ...
    // Set HPF corner
    R850_I2C.RegAddr = 24;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x0F, (HPF_Offset<<4));
    R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
...
R850_ErrCode R850_SetIfLpf(R850_TUNER_NUM R850_Tuner_Num, UINT32 LPF_Cor_Freq)
{
    ...
    // Set LPF Lsb bit
    R850_I2C.RegAddr = 23;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFE, R850_Lpf_Lsb);
    R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
    ...
    // Set LPF coarse BW
    R850_I2C.RegAddr = 23;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x9F, (R850_Bandwidth<<5));
    R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
...
R850_ErrCode R850_GetRfRssi(R850_TUNER_NUM R850_Tuner_Num, UINT32 RF_Freq_Khz, R850_Standard_Type RT_Standard, INT32 *RfLevelDbm, UINT8 *fgRfMaxGain)
{
    ...
    INT8 R850_Start_Gain_Cal_By_Freq_NORMAL_TYPE[7] = {10, -10, -30, -10, 20, 20, 20};
    INT8 R850_Start_Gain_Cal_By_Freq_ISDBT[5] = {0, -20, 0, +20, +30};
   ...
   //max gain indicator
    if((rf_gain_info.RF_gain1==31) && (rf_gain_info.RF_gain2==15) && (rf_gain_info.RF_gain3==15) && (rf_gain_info.RF_gain4==15))
        ...
        *fgRfMaxGain = 0;
    rf_limit = (((R850_GET_REG_Array(R850_Tuner_Num, 18) & 0x04) >> 1) + ((R850_GET_REG_Array(R850_Tuner_Num, 16) & 0x40) >> 6));
    if(rf_limit==0)
        rf_limit = 15;
    else if(rf_limit==1)
        rf_limit = 11;
    ...
    mixer_limit = (((R850_GET_REG_Array(R850_Tuner_Num, 22) & 0xC0)>>6)*2)+6;			//0=6, 1=8, 2=10, 3=12
    mixer_1315_gain = ((R850_GET_REG_Array(R850_Tuner_Num, 25) & 0x02)>>1);			//0:original, 1:ctrl by mixamp (>10)
    ...
    filter_limit = (R850_GET_REG_Array(R850_Tuner_Num, 22) & 0x01);
    if(filter_limit==1)
        filter_limit = 15;
    else
        filter_limit = 13;
    ...
    if(RT_Standard != R850_ISDB_T_IF_5M)
    {
        //coarse adjustment
...
//Method 2 : All frequencies are finely adjusted..
    ...
    if(rf_gain_info.RF_gain1 >= 10)
    {
        ...
        if(RT_Standard != R850_ISDB_T_IF_5M)
            acc_lna_gain += (UINT16)(Lna_Acc_Gain_offset_NORMAL_TYPE[u1LnaGainqFactorIdx]);
        else
            acc_lna_gain += (UINT16)(Lna_Acc_Gain_offset_ISDBT[u1LnaGainqFactorIdx]);
...
//Optimize value
    ...
    Dlg_I2C.RegAddr = 22;
    R850_SET_REG_Array(R850_Tuner_Num, Dlg_I2C.RegAddr, 0xF3, 0x08);
    Dlg_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, Dlg_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &Dlg_I2C) != RT_Success)
        return RT_Fail;

    //IF_AGC read, R18[5]=1
    Dlg_I2C.RegAddr = 18;
    R850_SET_REG_Array(R850_Tuner_Num, Dlg_I2C.RegAddr, 0xFF, 0x20);
    Dlg_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, Dlg_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &Dlg_I2C) != RT_Success)
        return RT_Fail;
    ...
    Dlg_I2C.RegAddr = 11;
    R850_SET_REG_Array(R850_Tuner_Num, Dlg_I2C.RegAddr, 0xFD, 0x00);
    Dlg_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, Dlg_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &Dlg_I2C) != RT_Success)
        return RT_Fail;
...
R850_ErrCode R850_SetXtalCap(R850_TUNER_NUM R850_Tuner_Num, UINT8 u8XtalCap)
{
    ...
    R850_I2C.RegAddr = 33;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x07, (Capx_3_0 << 3) | ( XtalCap << 7));
    R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
    R850_I2C.RegAddr = 34;
    R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xF7, (Capxx << 3));
    R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
    if(I2C_Write(R850_Tuner_Num, &R850_I2C) != RT_Success)
        return RT_Fail;
...
R850_ErrCode R850_SetHpfNotch(R850_TUNER_NUM R850_Tuner_Num, R850_HpfNotch_Type R850_HpfNotch)
{
    ...
    {
        R850_SET_REG_Array(R850_Tuner_Num, 23, 0x7F, 0x00);  //R23[7]=0
    }
    else //gain attenuation
    {
        R850_SET_REG_Array(R850_Tuner_Num, 23, 0xFF, 0x80);
    }
...
R850_ErrCode R850_AtscMonitor(R850_TUNER_NUM R850_Tuner_Num, UINT32 Freq_KHz)
{
    UINT8 filter_gain = 0;
    UINT8 mixer_gain = 0;

    if(*(pR850_Atsc_Mode +  R850_Tuner_Num)==0) //fast
    {
        *(pR850_Atsc_Mode +  R850_Tuner_Num) = 1; //slow_narrow
    }

    if((Freq_KHz>500000) && (Freq_KHz<600000))
    {
        ...
        filter_gain = (R850_I2C_Len.Data[5] & 0x0F);          //filter
        mixer_gain = (R850_I2C_Len.Data[4] & 0xF0)>>4;        //mixer

        //monitor in which mode
        if(filter_gain<=4)  //mode2 (slow_wide)
        {
            if(*(pR850_Atsc_Mode2_Count +  R850_Tuner_Num)<3)
            {
                *(pR850_Atsc_Mode2_Count +  R850_Tuner_Num)+=1;
            }

            if(*(pR850_Atsc_Mode2_Count +  R850_Tuner_Num)>=2)
            {
                *(pR850_Atsc_Mode +  R850_Tuner_Num) = 2;
                *(pR850_Atsc_Mode1_Count +  R850_Tuner_Num) = 0; //clear mode1 count
            }
        }
        else if(filter_gain>8)  //mode1 (slow_narrow)
        {
            if(*(pR850_Atsc_Mode1_Count +  R850_Tuner_Num)<3)
            {
                *(pR850_Atsc_Mode1_Count +  R850_Tuner_Num)+=1;
            }

            if(*(pR850_Atsc_Mode1_Count +  R850_Tuner_Num)>=2)
            {
                *(pR850_Atsc_Mode +  R850_Tuner_Num) = 1;
                *(pR850_Atsc_Mode2_Count +  R850_Tuner_Num) = 0; //clear mode2 count
            }
        }
    ...
    if(*(pR850_Atsc_Mode +  R850_Tuner_Num) != *(pR850_Atsc_Mode_Pre +  R850_Tuner_Num))
    {
        if(*(pR850_Atsc_Mode +  R850_Tuner_Num)==0) //fast
        {
            // HPF = pR850_Atsc_HPF R24[7:4]
            R850_SET_REG_Array(R850_Tuner_Num, 24, 0x0F, (*(pR850_Atsc_HPF + R850_Tuner_Num)<<4));    //HPF set R850_Atsc_HPF

            // LPF = pR850_Atsc_LPF R23[4:0]
            R850_SET_REG_Array(R850_Tuner_Num, 23, 0xE0, *(pR850_Atsc_LPF + R850_Tuner_Num));    //LPF set R850_Atsc_LPF

            // LNA VTL/H = 0.84/1.34
            R850_I2C.RegAddr = 39;
            R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x00, 0x5A);

            // RF VTL/H = 0.94/1.44
            R850_I2C.RegAddr = 42;
            R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x00, 0x6B);

            // MIXER VTL = 0.74
            R850_I2C.RegAddr = 43;
            R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xF0, 0x04);
            // MIXER VTH = 1.44
            R850_I2C.RegAddr = 41;
            R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xF0, 0x0B);
            ...
            // Mixer Gain Limt = 10
            R850_I2C.RegAddr = 22;
            R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x3F, 0x80);

            //LNA_RF_dis current = normal
            R850_I2C.RegAddr = 13;
            R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xDF, 0x20);


            //LNA_RF Discharge Mode = both(fast+slow)
            R850_I2C.RegAddr = 45;
            R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFC, 0x03);
            R850_I2C.RegAddr = 31;
            R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xFE, 0x01);
            R850_I2C.RegAddr = 32;
            R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xDF, 0x20);

        }
        else if(*(pR850_Atsc_Mode +  R850_Tuner_Num)==1)  //slow_narrow
        {
            if(*(pR850_Atsc_Mode_Pre +  R850_Tuner_Num)==2)  //slow_wide -> slow_narrow
            {
                if(mixer_gain>=6)
                {
                     //Mixer off (R19[5]=0)
                     ...
                     R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xDF, 0x00);
                     R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
                     I2C_Write(R850_Tuner_Num, &R850_I2C);

                     //Mixer gain=7
                     R850_I2C.RegAddr = 41;
                     R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xF0, 0x07);
                     R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
                     I2C_Write(R850_Tuner_Num, &R850_I2C);
                }
            }
            ...
            if((Freq_KHz>500000) && (Freq_KHz<600000))
            {
                // HPF = R850_Atsc_HPF R24[7:4]
                if(*(pR850_Atsc_HPF + R850_Tuner_Num) >= 13)
                {
                    R850_SET_REG_Array(R850_Tuner_Num, 24, 0x0F, 0xF0);
                }
                else
                {
                    R850_SET_REG_Array(R850_Tuner_Num, 24, 0x0F, ((*(pR850_Atsc_HPF + R850_Tuner_Num)+2)<<4));
                }

                 // LPF = R850_Atsc_LPF R23[4:0]
                if(*(pR850_Atsc_LPF + R850_Tuner_Num) <= 3)
                {
                    R850_SET_REG_Array(R850_Tuner_Num, 23, 0xE0, 0x00);
                }
                else
                {
                    R850_SET_REG_Array(R850_Tuner_Num, 23, 0xE0, (*(pR850_Atsc_LPF + R850_Tuner_Num)-3));    //LPF set R850_Atsc_LPF-3
                }
            }  //end if(freq_KHz)
            else
            {
        ...
        {
            if(*(pR850_Atsc_Mode_Pre +  R850_Tuner_Num)==1)  //slow_narrow -> slow_wide
            {
                if(mixer_gain>=8)
                {
                     //Mixer off (R19[5]=0)
            ...
            if((Freq_KHz>500000) && (Freq_KHz<600000))
            {
                ...
                if(*(pR850_Atsc_HPF + R850_Tuner_Num) >= 13)
                {
                    R850_SET_REG_Array(R850_Tuner_Num, 24, 0x0F, 0xF0);    //HPF set 15
                }
                else
                {
                    R850_SET_REG_Array(R850_Tuner_Num, 24, 0x0F, ((*(pR850_Atsc_HPF + R850_Tuner_Num)+2)<<4));    //HPF set R850_Atsc_HPF+2
                }

                 // LPF = R850_Atsc_LPF R23[4:0]
                if(*(pR850_Atsc_LPF + R850_Tuner_Num) <= 3)
                {
                    R850_SET_REG_Array(R850_Tuner_Num, 23, 0xE0, 0x00);    //LPF set 0
                }
                else
                {
                    R850_SET_REG_Array(R850_Tuner_Num, 23, 0xE0, (*(pR850_Atsc_LPF + R850_Tuner_Num)-3));    //LPF set R850_Atsc_LPF-3
            ...
            {
                 // HPF = R850_Atsc_HPF R24[7:4]
                R850_SET_REG_Array(R850_Tuner_Num, 24, 0x0F, (*(pR850_Atsc_HPF + R850_Tuner_Num)<<4));

                // LPF = R850_Atsc_LPF R23[4:0]
                R850_SET_REG_Array(R850_Tuner_Num, 23, 0xE0, *(pR850_Atsc_LPF + R850_Tuner_Num));
            }


            // LNA VTL/H = 0.64/1.44
            R850_I2C.RegAddr = 39;
            R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x00, 0x3B);

            // RF VTL/H = 0.74/1.44
            R850_I2C.RegAddr = 42;
            R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0x00, 0x4B);

            // MIXER VTL = 0.64
            R850_I2C.RegAddr = 43;
            R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xF0, 0x03);
            // MIXER VTH = 1.44
            R850_I2C.RegAddr = 41;
            R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xF0, 0x0B);
            ...
            //LNA_RF_dis current = 1/3 dis current
            R850_I2C.RegAddr = 13;
            R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xDF, 0x00);
         ...
         R850_I2C.RegAddr = 22;
         R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);  //max_gain_limit, move to last I2C_write
         I2C_Write(R850_Tuner_Num, &R850_I2C);
        ...
        //Auto Mixer AGC (R19[5]=1)
        R850_I2C.RegAddr = 19;
        R850_SET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr, 0xDF, 0x20);
        R850_I2C.Data = R850_GET_REG_Array(R850_Tuner_Num, R850_I2C.RegAddr);
        I2C_Write(R850_Tuner_Num, &R850_I2C);

    } //end if(mode!=mode_pre)

    *(pR850_Atsc_Mode_Pre +  R850_Tuner_Num) = *(pR850_Atsc_Mode +  R850_Tuner_Num);
...
R850_ErrCode R850_Dump_Data(R850_TUNER_NUM R850_Tuner_Num)
{
    ...
    printf("IMG_flag=%d, Pulse_flag=%d\n", R850_IMGFlag, R850_PulseFlag);

    for(i=0;i<R850_REG_NUM;i++)
    {
        printf("R%d=0x%X, ",i,R850_I2C_Len.Data[i]);
    }
    printf("\n");
    */
    return RT_Success;

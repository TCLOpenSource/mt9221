...
void SI2151_OEM_I2C_channel_setting(U8 bus_num)
{
    tuner_i2c_channel_bus = bus_num;
    printTun_dev("tuner_i2c_channel [%d] \n",tuner_i2c_channel_bus);
}
...
#else
    //use bus channel 0x1
    ret = MDrv_SW_IIC_ReadBytes(tuner_i2c_channel_bus ,aSlaveAddr, 0, 0, aDataSize, apData);
    if (ret != 0)
        printTun_dev("SI215X_ucI2cReadOnly fail !!\n");
#endif
    // -----> Print IIC data
...
#else
    ret = MDrv_SW_IIC_WriteBytes(tuner_i2c_channel_bus, aSlaveAddr, 0, 0, aDataSize, apData);
    if (ret != 0)
        printTun_dev("SI215X_ucI2cWriteOnly fail !!\n");
#endif
// -----> Print IIC data
    printTun_dev(("<0x%02x> : ",  aSlaveAddr));
    for( i = 0; i < aDataSize ; i++ ){

...
#define AV2012_I2C_ADDR                       ((U8)  0xC4)
...
static MS_U8 platform_gpio_control(MS_U8 high)
{
    MS_U8 bRet = 1;
    mapi_gpio *gptr = NULL;
    // gptr = mapi_gpio::GetGPIO_Dev(DVBS_TUNER_RST);
...
static BOOL AV2012_WriteReg(U8 u8SlaveID, U8 u8Addr, U8 u8Data)
{
    BOOL bRet;
    U8 u8Value[2];
    u8Value[0]=u8Addr;
...
static BOOL AV2012_ReadReg(U8 u8SlaveID, U8 u8Addr, U8 *u8Data)
{
    BOOL bRet;
...
static void AV2012_SlaveID_Check(void)
{
     U8 regValue;
...
// input IF_AGC,
// return dBm.
static float MDrv_DVBS_Tuner_Get_RSSI(U16 u16_gain, U8 Dtype)
{
    U8 indx = 0;
    U8 arry_size = 0;
    float rf_dbm = 0.0;
    S_RSSI_TABLE *rssi_av2012;

   switch(Dtype)
   {
        case DEMOD_EXTEND_msb131x:
            arry_size = sizeof(rssi_av2012_131x)/sizeof(S_RSSI_TABLE);
            rssi_av2012 = rssi_av2012_131x;
            break;
        case DEMOD_EXTEND_msb1240:
            arry_size = sizeof(rssi_av2012_1240)/sizeof(S_RSSI_TABLE);
            rssi_av2012 = rssi_av2012_1240;
            break;
        default:
            arry_size = sizeof(rssi_av2012_131x)/sizeof(S_RSSI_TABLE);
            rssi_av2012 = rssi_av2012_131x;
    ...
    if (u16_gain > rssi_av2012[0].u16_if_agc)
    {
        rf_dbm = (float)(u16_gain - rssi_av2012[1].u16_if_agc)/(float)(rssi_av2012[0].u16_if_agc - rssi_av2012[1].u16_if_agc)*(rssi_av2012[0].LvdBm - rssi_av2012[1].LvdBm)+rssi_av2012[1].LvdBm;
    }
    else if (u16_gain <= rssi_av2012[arry_size-1].u16_if_agc)
    {
        rf_dbm = (float)(u16_gain - rssi_av2012[arry_size-2].u16_if_agc)/(float)(rssi_av2012[arry_size-1].u16_if_agc - rssi_av2012[arry_size-2].u16_if_agc)*(rssi_av2012[arry_size-1].LvdBm - rssi_av2012[arry_size-2].LvdBm)+rssi_av2012[arry_size-2].LvdBm;
    }
    else
    {
        for(indx = 0;indx<arry_size;indx++)
        {
            if (u16_gain > rssi_av2012[indx].u16_if_agc) break;
        }

        if (indx < arry_size)
        {
            rf_dbm = (float)(u16_gain - rssi_av2012[indx].u16_if_agc)/(float)(rssi_av2012[indx-1].u16_if_agc - rssi_av2012[indx].u16_if_agc)*(rssi_av2012[indx-1].LvdBm - rssi_av2012[indx].LvdBm)+rssi_av2012[indx].LvdBm;
        }
        //Fix coverity issue. Make sure that indx will not overrun out of arry_size.
        else
        {
            //This assignment is same as "else if (u16_gain <= rssi_av2012[arry_size-1].u16_if_agc)" did
            rf_dbm = (float)(u16_gain - rssi_av2012[arry_size-2].u16_if_agc)/(float)(rssi_av2012[arry_size-1].u16_if_agc - rssi_av2012[arry_size-2].u16_if_agc)*(rssi_av2012[arry_size-1].LvdBm - rssi_av2012[arry_size-2].LvdBm)+rssi_av2012[arry_size-2].LvdBm;
        }
    }
...
int MDrv_TUNER_DVBS_SetTune(int minor, U16 u16CenterFreqMHz, U32 u32SymbolRateKs)
{
    ...
    {
        DBG_TUNER(TUNER_PRINT("AIROHA_AV2012 auto-scan mode\n"));
        bAutoScan = TRUE;
    }
    ...
    if (bRet == false)
    {
        TUNER_PRINT("AIROHA_AV2012_ConfigTuner fail\n");
    }
...
int MDrv_TUNER_TunerInit(int minor,s_Tuner_dev_info *devTunerInit)
{
    ...
    if (minor < MAX_TUNER_DEV_NUM)
    {
        ...
        || (p_dev->e_status == E_SUSPEND) )
        {
            ...
            if (bRet != TRUE)
            {
                TUNER_PRINT("AV2012 Tuner_Register_Initial fail\n");
            }
            else
            {
...
int MDrv_TUNER_IsLocked(int minor)
{
    ...
    if (bRet==FALSE)
    {
        TUNER_PRINT("MDrv_TUNER_IsLocked fail\n");
        return bRet;
    }
    else
    {
    ...
    //TUNER_PRINT("AV2012 Tuner Status 0x%x\n", u8Data);
    return bRet;
}

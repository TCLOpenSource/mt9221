...
#define DFS_ROOT_NAME	"iaxxx"
#define DFS_MODE	0600
...
static int iaxxx_dfs_open(struct iaxxx_dbgfs_data *node_data, struct file *file)
{
	...
	trans = iaxxx_kvmalloc(sizeof(*trans), __GFP_ZERO);
	if (!trans)
		return -ENOMEM;

	/* Allocate log buffer */
	log = iaxxx_kvmalloc(logbufsize, __GFP_ZERO);
	if (!log) {
		kvfree(trans);
				  ...
				  size_t count, loff_t *ppos)
{
	...
	kbuf = iaxxx_kvmalloc(count + 1, __GFP_ZERO);
	if (!kbuf)
		return -ENOMEM;
	...
	*ppos += count;
	...
	if (ret) {
		pr_err("Register write failed : %zu", ret);
	} else {
		ret = count;
				 ...
				 size_t count, loff_t *ppos)
{
	...
	if (ret == len) {
		pr_err("copy register values to user failed");
		return -EFAULT;
	}
...
static struct dentry *iaxxx_dfs_create_fs(void)
{
	...
	if (IS_ERR(root)) {
		pr_err("creating top level directory failed : %ld",
		       (long)root);
				...
				struct regmap *map, void **dfs_node)
{
	...
	pr_debug("Adding register map debugfs for %s", dev_name(dev));
	root = iaxxx_dfs_get_root();
	if (!root)
		return -ENOENT;

	/* Allocate transaction data for the controller */
	node_data = iaxxx_kvmalloc(sizeof(*node_data), __GFP_ZERO);
	if (!node_data)
		return -ENOMEM;

	dir = debugfs_create_dir(dev->kobj.name, root);
	if (!dir) {
		pr_err("Error creating regmap entry for %s", dev_name(dev));
...
int iaxxx_dfs_del_regmap(struct device *dev, struct regmap *map)
{
	...
	pr_debug("Deleting register map debugfs for %s", dev_name(dev));
	...
	rc = -EINVAL;
	pr_debug("Unknown register map for %s", dev_name(dev));

	...
	fusion_list_foreach(l, dev->fusionee.list) {
		Fusionee *fusionee = (Fusionee *) l;
               seq_printf(m,
                       ...
                       fusionee->messages.count, atomic_long_read((atomic_long_t *) &fusionee->rcv_total),
                       atomic_long_read((atomic_long_t *)  &fusionee->snd_total)
		      ...
		      Fusionee * fusionee, void *buf, int buf_size, bool block)
{
	...
	while (fusionee->messages.count) {
		...
		if(is_compat_task())
		{
			...
			{
			      fcm = message->data;
				cfcm.handler = ptr_to_compat(fcm->handler);
				cfcm.ctx = ptr_to_compat(fcm->ctx);
				cfcm.caller = fcm->caller;
				cfcm.call_arg = fcm->call_arg;
				cfcm.call_ptr = ptr_to_compat(fcm->call_ptr);

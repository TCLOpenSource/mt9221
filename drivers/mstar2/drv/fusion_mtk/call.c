...
static void print_call( FusionCall* call )
{
     ...
     printk( KERN_CRIT "%-2d %s, fid:%llu, %d calls)",
             entry->id,
                    ...
                    FusionCallExecute * execute)
{
     ...
     if (execution) {
          ...
          if (signal_pending(current) || execution->is_executed_return == false)
          {
                ...
                printk( KERN_EMERG "FusionIPC: Warning signal_pending(current) -> exit from fusion_call_execute() without FCEF_RESUMABLE. current->pid=%d\n", current->pid );
                ...
                if (!(execute->flags & FCEF_RESUMABLE)) {
                     ...
                     printk( KERN_EMERG "FusionIPC: Warning signal_pending(current) -> exit from fusion_call_execute() without FCEF_RESUMABLE. current->pid=%d, fid=0x%08x, call->fusion_id=%llu\n",
                             current->pid,
...
fusion_call_return(FusionDev * dev, int fusion_id, FusionCallReturn * call_ret)
{
    ...
    FUSION_ASSERT( call->executions != NULL );

...
print_skirmish_internal( FusionSkirmish* skirmish, const char* pHeader )
{
     ...
     printk( "%s %s %-3ld.%03ld %d %-18s [1] t:%llu f:%llu fpid:%-4d c:%d s:%d [2] t:%llu f:%llu fpid:%-4d c:%d s:%d - c:%d f:%d p:%-4d w:%d\n",
             pHeader ? pHeader : " ",
             p,
...
#endif
     seq_printf(p, "[1] t:%llu, f:%llu, fpid:%d, c:%d s:%d, [2] t:%llu, f:%llu, fpid:%d, c:%d, s:%d",
                skirmish->transfer_to,
                skirmish->transfer_from,
                skirmish->transfer_from_pid,
                skirmish->transfer_count,
...
int fusion_skirmish_prevail(FusionDev * dev, int id, int fusion_id)
{
     ...
     while (   skirmish->lock_pid
          ...
          for(i=0; i<5; i++)
          {
          	 if(skirmish->wait_pid[i] == 0)
			 	 break;
          }
		  if(5 == i)
		  	i=0;
		  	skirmish->wait_pid[i] = fusion_core_pid( fusion_core );
          ret = fusion_skirmish_wait(skirmish, NULL);
          if (ret)
               return ret;
		  skirmish->wait_pid[i] = 0;
     }
